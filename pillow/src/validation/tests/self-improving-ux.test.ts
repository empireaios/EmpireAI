...
describe("T5-09 Self-Improving UX Engine", () => {
  beforeEach(() => {
    resetVisualCaptureEngineForTesting();
    resetUiStateMapperForTesting();
    resetComponentRecognitionForTesting();
  ...
  test("buildSelfImprovingUxConfiguration loads defaults", () => {
    const config = buildSelfImprovingUxConfiguration(REPO_ROOT);
    assert.equal(config.enabled, true);
    assert.equal(config.learnOnlyMode, true);
    assert.equal(config.learningRulesEnabled, true);
    assert.ok(LEARNING_CATEGORIES.includes("redesign_learning"));
  });

  test("self-improving UX engine initializes with doctrine doc", async () => {
    const engines = await buildPipeline();
    const state = engines.selfImprovingUx.getState();
    assert.equal(state.engineVersion, "PILLOW-SIUX-001");
    assert.equal(state.missionId, "T5-09");
    assert.ok(SELF_IMPROVING_UX_SYSTEM_PATH.includes("SELF_IMPROVING_UX"));
    engines.visualCapture.stopCapture();
  });

  test("learnUx produces machine-readable UX learning records", async () => {
    const engines = await buildPipeline();
    ...
    assert.ok(report.learningRunReportId.startsWith("siux-run-"));
    assert.ok(report.records.length > 0, `expected records, got ${report.records.length}`);
    for (const record of report.records) {
      assert.equal(record.learnOnly, true);
      assert.ok(record.learningId.startsWith("siux-"));
      assert.ok(record.metadataVersion);
      assert.ok(record.learnedUxInsight);
      assert.ok(record.recommendationImprovement);
      assert.ok(record.prioritizationImprovement);
    }
    engines.visualCapture.stopCapture();
  });

  test("learns from redesign and workspace intelligence", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.selfImprovingUx.learnUx();
    assert.ok(
      report.records.some(
        (r) =>
          r.learningCategory === "redesign_learning" ||
          r.learningCategory === "workspace_learning" ||
          r.learningCategory === "continuous_ux_intelligence_learning",
      ),
    );
    engines.visualCapture.stopCapture();
  });

  test("learns from workflow and productivity outcomes", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.selfImprovingUx.learnUx();
    assert.ok(
      report.records.some(
        (r) =>
          r.learningCategory === "workflow_learning" ||
          r.learningCategory === "productivity_learning",
      ),
    );
    engines.visualCapture.stopCapture();
  });

  test("improves future recommendations and prioritization", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.selfImprovingUx.learnUx();
    assert.ok(
      report.records.some(
        (r) => r.recommendationImprovement.length > 0 && r.prioritizationImprovement.length > 0,
      ),
    );
    engines.visualCapture.stopCapture();
  });

  test("never applies UX changes automatically (learn-only)", async () => {
    const engines = await buildPipeline();
    const report = engines.selfImprovingUx.learnUx();
    for (const record of report.records) {
      assert.equal(record.learnOnly, true);
  ...
  test("start and stop continuous learning toggles monitoring state", async () => {
    const engines = await buildPipeline();
    const started = engines.selfImprovingUx.startContinuousLearning();
    assert.equal(started.health.continuousLearningActive, true);
    const stopped = engines.selfImprovingUx.stopContinuousLearning();
    assert.equal(stopped.health.continuousLearningActive, false);
    engines.visualCapture.stopCapture();
  });
  ...
  test("validateForSupervisorSync reports readiness after learning", async () => {
    ...
    const sync = engines.selfImprovingUx.validateForSupervisorSync();
    assert.ok(sync.readinessScore >= 20);
    assert.ok(["healthy", "degraded", "blocked"].includes(sync.health));
    engines.visualCapture.stopCapture();
  });

  test("cockpit snapshot exposes learning health", async () => {
    const engines = await buildPipeline();
    engines.selfImprovingUx.learnUx();
    const cockpit = engines.selfImprovingUx.getCockpitSnapshot();
    assert.ok(cockpit.totalLearningCycles >= 1);
  ...
  test("getLatestReport returns most recent learning run", async () => {
    const engines = await buildPipeline();
    const report = engines.selfImprovingUx.learnUx();
    const latest = engines.selfImprovingUx.getLatestReport();
    assert.equal(latest?.learningRunReportId, report.learningRunReportId);
    engines.visualCapture.stopCapture();
  });

  test("updates experience knowledge base after learning", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.selfImprovingUx.learnUx();
    assert.ok(report.knowledgeEntries.length > 0);
    const kb = engines.selfImprovingUx.getKnowledgeBase();
    assert.ok(kb.length > 0);
    assert.ok(kb[0]!.entryId.startsWith("siux-kb-"));
    engines.visualCapture.stopCapture();
  });
});
