...
describe("T5-03 UX Opportunity Discovery", () => {
  beforeEach(() => {
    resetVisualCaptureEngineForTesting();
    resetUiStateMapperForTesting();
    resetComponentRecognitionForTesting();
  ...
  test("buildUxOpportunityDiscoveryConfiguration loads defaults", () => {
    const config = buildUxOpportunityDiscoveryConfiguration(REPO_ROOT);
    assert.equal(config.enabled, true);
    assert.equal(config.discoverOnlyMode, true);
    assert.equal(config.prioritizationRulesEnabled, true);
    assert.ok(OPPORTUNITY_CATEGORIES.includes("layout_improvement"));
  });

  test("UX opportunity discovery initializes with doctrine doc", async () => {
    const engines = await buildPipeline();
    const state = engines.uxOpportunityDiscovery.getState();
    assert.equal(state.engineVersion, "PILLOW-UOD-001");
    assert.equal(state.missionId, "T5-03");
    assert.ok(UX_OPPORTUNITY_DISCOVERY_SYSTEM_PATH.includes("UX_OPPORTUNITY_DISCOVERY"));
    engines.visualCapture.stopCapture();
  });

  test("discover produces machine-readable opportunity records", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.uxOpportunityDiscovery.discover();
    assert.ok(report.discoveryRunReportId.startsWith("uod-run-"));
    assert.notEqual(report.validation.decision, "fail");
    for (const opp of report.opportunities) {
      assert.equal(opp.discoverOnly, true);
      assert.ok(opp.opportunityId.startsWith("uod-"));
      assert.ok(opp.metadataVersion);
    }
    engines.visualCapture.stopCapture();
  });

  test("discovers loading state opportunities from audit chain", async () => {
    const engines = await buildPipeline();
    engines.continuousScreenObservation.observe({
      uiSnapshot: {
        screenId: "screen-loading",
        routeOrViewId: "/loading",
        surfaceStates: ["loading"],
      },
    });
    engines.autonomousUxAudit.audit();
    const report = engines.uxOpportunityDiscovery.discover();
    assert.ok(
      report.opportunities.some((o) => o.opportunityCategory === "performance_related_ux_improvement"),
    );
    engines.visualCapture.stopCapture();
  });

  test("prioritizes and ranks opportunities by impact", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.uxOpportunityDiscovery.discover();
    const ranked = engines.uxOpportunityDiscovery.getTopOpportunities();
    assert.equal(ranked.length, report.opportunities.length);
    if (ranked.length >= 2) {
      const priorities = ["critical", "high", "medium", "low", "deferred"];
      assert.ok(
        priorities.indexOf(ranked[0]!.priority) <= priorities.indexOf(ranked[1]!.priority),
      );
    }
    engines.visualCapture.stopCapture();
  });

  test("never applies UX changes automatically (discover-only)", async () => {
    const engines = await buildPipeline();
    const report = engines.uxOpportunityDiscovery.discover();
    for (const opp of report.opportunities) {
      assert.equal(opp.discoverOnly, true);
  ...
  test("start and stop continuous discovery toggles monitoring state", async () => {
    const engines = await buildPipeline();
    const started = engines.uxOpportunityDiscovery.startContinuousDiscovery();
    assert.equal(started.health.continuousDiscoveryActive, true);
    const stopped = engines.uxOpportunityDiscovery.stopContinuousDiscovery();
    assert.equal(stopped.health.continuousDiscoveryActive, false);
    engines.visualCapture.stopCapture();
  });

  test("validateForSupervisorSync reports readiness after discovery", async () => {
    const engines = await buildPipeline();
    engines.uxOpportunityDiscovery.discover();
    const sync = engines.uxOpportunityDiscovery.validateForSupervisorSync();
    assert.ok(sync.readinessScore >= 20);
    assert.ok(["healthy", "degraded", "blocked"].includes(sync.health));
    engines.visualCapture.stopCapture();
  });

  test("cockpit snapshot exposes discovery health", async () => {
    const engines = await buildPipeline();
    engines.uxOpportunityDiscovery.discover();
    const cockpit = engines.uxOpportunityDiscovery.getCockpitSnapshot();
    assert.ok(cockpit.totalDiscoveries >= 1);
    assert.ok(cockpit.engineStatus);
    engines.visualCapture.stopCapture();
  });

  test("getLatestReport returns most recent discovery run", async () => {
    const engines = await buildPipeline();
    const report = engines.uxOpportunityDiscovery.discover();
    const latest = engines.uxOpportunityDiscovery.getLatestReport();
    assert.equal(latest?.discoveryRunReportId, report.discoveryRunReportId);
    engines.visualCapture.stopCapture();
  });
});
