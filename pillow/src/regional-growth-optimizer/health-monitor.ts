/** X4-14 — Regional Growth Optimizer health monitoring. */
...
export class HealthMonitor {
  ...
  buildReport(input: {
    ...
    record: RegionalGrowthOptimizerEngineRecord | null;
    ...
    const notes: string[] = [];
    if (!input.config.enabled) notes.push("Regional Growth Optimizer disabled");
    if (input.consecutiveFailures > 0) {
