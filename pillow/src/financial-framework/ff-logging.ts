/** R3-01 — Financial Framework logging. */
...
const SENSITIVE_PATTERN =
  /(token|secret|password|api[_-]?key|authorization|bearer|credential|account[_-]?number|iban|routing)/i;
...
export function appendFrameworkLog(input: {
  ...
  const entry: FrameworkLogEntry = {
    logId: `ff-log-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
