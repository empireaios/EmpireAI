/**
 * Integer-minor-unit money model for the Cashflow Worker (Q9-03).
 *
 * All cashflow arithmetic (add/sub/compare) operates exclusively on integer
 * minor units (e.g. cents for SGD/USD) via the helpers below. `Math.round` is
 * only ever applied at the single decimal→minor-units parse boundary
 * (`moneyFromDecimal`) when converting a verified accounting decimal amount
 * into minor units. No monetary total is ever computed with floating-point
 * multiplication/division — `moneyToDecimalNumber`/`moneyToDecimalString`
...
function assertSameCurrency(a: MoneyMinor, b: MoneyMinor): void {
  if (a.currency !== b.currency) {
    throw new Error(
      `Cashflow Worker cannot combine mismatched currencies without conversion: ${a.currency} vs ${b.currency}`,
    );
  }
}
...
/**
 * Parse a verified accounting decimal amount (major units, e.g. 10.50) into
 * integer minor units (1050). This is the ONLY boundary where a rounding
 * operation on a decimal value is permitted. String inputs are parsed
 * digit-by-digit to avoid floating-point representation error entirely;
 * number inputs use a single `Math.round` at this boundary only.
 */
