/** R5-10 — Marketing Analytics Dashboard logging. */
...
function sanitize(details: string): string {
  if (SENSITIVE_PATTERN.test(details)) {
    return "[redacted — sensitive marketing credential omitted]";
  }
  return details;
}

export function appendMadLog(input: {
  event: string;
  level: DashboardLogEntry["level"];
  details: string;
}): DashboardLogEntry {
  const entry: DashboardLogEntry = {
    logId: `mad-log-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
...
export function resetMadLogsForTesting(): void {
  logs.length = 0;
}
