...
async function buildPipeline() {
  ...
  return {
    ...
    autonomousBuilderCertification,
    naturalUxConversation,
    voiceUxCommands, screenAnnotation, visualFoundation,
  };
}
...
describe("T4-03 Screen Annotation", () => {
  beforeEach(() => {
    resetVisualCaptureEngineForTesting();
    resetUiStateMapperForTesting();
    resetComponentRecognitionForTesting();
  ...
  test("buildScreenAnnotationConfiguration loads defaults", () => {
    const config = buildScreenAnnotationConfiguration(REPO_ROOT);
    assert.equal(config.enabled, true);
    assert.equal(config.annotationCaptureRulesEnabled, true);
    assert.ok(config.supportedAnnotationTypes.length === ANNOTATION_TYPES.length);
  });

  test("screen annotation engine initializes with doctrine doc", async () => {
    const { screenAnnotation, visualCapture } = await buildPipeline();
    const state = screenAnnotation.getState();
    assert.equal(state.engineVersion, "PILLOW-SA-001");
    assert.equal(state.missionId, "T4-03");
    assert.ok(state.configuration);
    assert.ok(SCREEN_ANNOTATION_SYSTEM_PATH.includes("SCREEN_ANNOTATION"));
    visualCapture.stopCapture();
  });

  test("annotate captures point and generates point-and-edit intent", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.screenAnnotation.annotate({
      annotationType: "point",
      pointerCoordinates: { x: 120, y: 80 },
      annotationText: "Fix spacing here",
    });
    assert.ok(report.annotationRunReportId.startsWith("sa-run-"));
    assert.ok(report.latestAnnotation);
    assert.equal(report.latestAnnotation!.annotationType, "point");
    assert.ok(report.latestIntent);
    assert.ok(report.latestIntent!.requestedEditSummary.length > 0);
    assert.ok(report.latestAnnotation!.confidenceScore > 0);
    engines.visualCapture.stopCapture();
  });

  test("component selection maps explicit component references", async () => {
    const engines = await buildPipeline();
    const report = engines.screenAnnotation.annotate({
      annotationType: "component_selection",
      referencedComponentIds: ["btn-submit", "form-settings"],
      annotationText: "These buttons need better contrast",
    });
    assert.ok(report.latestAnnotation!.referencedComponentIds.includes("btn-submit"));
    assert.ok(report.latestIntent!.targetComponentIds.includes("btn-submit"));
    engines.visualCapture.stopCapture();
  });

  test("ux complaint and design preference notes are understood", async () => {
    const engines = await buildPipeline();
    const complaint = engines.screenAnnotation.annotate({
      annotationType: "ux_complaint_note",
      pointerCoordinates: { x: 50, y: 50 },
      annotationText: "This sidebar is confusing",
    });
    assert.ok(complaint.latestIntent!.uxConcernSummary);
    const preference = engines.screenAnnotation.annotate({
      annotationType: "design_preference_note",
      annotationText: "Prefer darker theme on this panel",
    });
    assert.ok(preference.latestIntent!.designPreferenceSummary);
    engines.visualCapture.stopCapture();
  });

  test("annotations do not apply or approve changes automatically", async () => {
    const engines = await buildPipeline();
    const report = engines.screenAnnotation.annotate({
      annotationType: "edit_instruction",
      screenRegionBounds: { x: 10, y: 10, width: 200, height: 100 },
      annotationText: "Resize this card",
    });
    assert.notEqual(report.validation.decision, "fail");
    assert.ok(report.latestIntent);
    assert.ok(Array.isArray(report.latestIntent!.linkedBuilderCapabilities));
    engines.visualCapture.stopCapture();
  });

  test("ambiguous annotation may request clarification", async () => {
    const engines = await buildPipeline();
    const report = engines.screenAnnotation.annotate({
      annotationType: "point",
    });
    assert.ok(report.latestAnnotation);
    assert.ok(
      report.latestAnnotation!.processingStatus === "awaiting_clarification" ||
        report.latestIntent?.clarificationRequirement !== null ||
        report.validation.decision === "partial",
    );
    engines.visualCapture.stopCapture();
  });

  test("validateForSupervisorSync reports readiness after annotation", async () => {
    const engines = await buildPipeline();
    engines.screenAnnotation.annotate({
      annotationType: "review_request",
      pointerCoordinates: { x: 300, y: 200 },
      annotationText: "Review this dashboard section",
    });
    const sync = engines.screenAnnotation.validateForSupervisorSync();
    assert.ok(sync.readinessScore >= 20);
    assert.ok(["healthy", "degraded", "blocked"].includes(sync.health));
    engines.visualCapture.stopCapture();
  });

  test("cockpit snapshot exposes annotation status", async () => {
    const engines = await buildPipeline();
    engines.screenAnnotation.annotate({
      annotationType: "highlight",
      screenRegionBounds: { x: 0, y: 0, width: 400, height: 300 },
    });
    const cockpit = engines.screenAnnotation.getCockpitSnapshot();
    assert.ok(cockpit.totalAnnotations >= 1);
    assert.ok(cockpit.intentsGenerated >= 1);
    engines.visualCapture.stopCapture();
  });

  test("getLatestReport returns most recent annotation run", async () => {
    const engines = await buildPipeline();
    const report = engines.screenAnnotation.annotate({
      annotationType: "validation_request",
      annotationText: "Validate accessibility of this form",
    });
    const latest = engines.screenAnnotation.getLatestReport();
    assert.equal(latest?.annotationRunReportId, report.annotationRunReportId);
    engines.visualCapture.stopCapture();
  });
});
