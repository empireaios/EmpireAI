import { DIGITAL_PRODUCTS_FACTORY_CORE_IDENTITY } from "./paths.js";
import type {
...
/** Optional live integrations for Q5-01 Digital Products Factory Core. */
export type DigitalProductsFactoryCoreDependencies = {
  workerRegistry?: {
...
export class IntegrationCoordinator {
  ...
  private deps: DigitalProductsFactoryCoreDependencies = {};

  bind(deps: DigitalProductsFactoryCoreDependencies = {}) {
    this.deps = { ...deps };
  }
  ...
  registerMission(mission: DigitalProductBusinessMission): {
    registered: boolean;
    ...
    try {
      const result =
        engine.receiveMissionPlan?.({
          missionId: mission.factoryMissionId,
          businessId: mission.businessId,
          phase: "digital_products_orchestration",
          status: "planned",
  ...
  submitReport(report: DigitalProductsFactoryReport): {
    submitted: boolean;
    ...
    const result = runtime.submitWorkerReport({
      ...
      missionId: "Q5-01",
      currentStatus: report.currentPipelineStage,
      progress: report.currentPipelineStage === "completed" ? 100 : 50,
      blockers: [],
      risks: [],
      evidence: [
        ...
        `fulfilment_status=${report.fulfilmentStatus}`,
        `mce=${report.missionCoordinationRef ?? "none"}`,
      ],
      nextAction: "await_downstream_digital_products_workers",
      completionStatus:
        report.currentPipelineStage === "completed" ? "completed" : "in_progress",
      reportType: "worker",
      validated: true,
      digitalProductsFactoryReport: report,
    });

    const executiveReportId =
      result?.records?.find((r) => r.reportId)?.reportId ?? `ert-dpf-${Date.now()}`;
    appendDpfLog({
      event: "submit_report",
  ...
  private provisionWorkerIdentity(workerId: string) {
    const identity = {
      ...
      workerType: DIGITAL_PRODUCTS_FACTORY_CORE_IDENTITY.workerType,
      ...
      skillProfile: [...DIGITAL_PRODUCTS_FACTORY_CORE_IDENTITY.skillProfile],
      approvedTools: [...DIGITAL_PRODUCTS_FACTORY_CORE_IDENTITY.approvedTools],
      authorityLevel: DIGITAL_PRODUCTS_FACTORY_CORE_IDENTITY.authorityLevel,
  ...
  private isBound(target: IntegrationTarget): boolean {
    switch (target) {
      ...
      case "digital_products_factory_core_validation":
        return true;
      default:
        return false;
    }
  ...
  private describe(target: IntegrationTarget, workerId: string, status: string): string {
    return `${target} integration ${status} for ${workerId}; orchestration-only digital products factory core under Pillow.`;
  }
}
