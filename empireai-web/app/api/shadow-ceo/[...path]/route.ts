...
async function proxyShadowCeo(
  pathSegments: string[],
  request: Request,
  method: string,
): Promise<Response> {
  const url = new URL(request.url);
  const backendPath = `/shadow-ceo/${pathSegments.join("/")}${url.search}`;
...
export async function GET(request: Request, context: RouteContext) {
  ...
  return proxyShadowCeo(path, request, "GET");
}

export async function POST(request: Request, context: RouteContext) {
  const { path } = await context.params;
  return proxyShadowCeo(path, request, "POST");
}
