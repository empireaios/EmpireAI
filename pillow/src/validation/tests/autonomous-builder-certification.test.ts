...
describe("T3-10 Autonomous Builder Certification", () => {
  beforeEach(() => {
    resetVisualCaptureEngineForTesting();
    resetUiStateMapperForTesting();
    resetComponentRecognitionForTesting();
  ...
  test("buildAutonomousBuilderCertificationConfiguration loads defaults", () => {
    const config = buildAutonomousBuilderCertificationConfiguration(REPO_ROOT);
    assert.equal(config.enabled, true);
    assert.equal(config.requireEndToEndPass, true);
    assert.equal(config.validationScope.length, 9);
    assert.deepEqual(config.validationScope, [...T3_MISSION_IDS]);
  });

  test("certification engine initializes with doctrine doc", async () => {
    const { autonomousBuilderCertification, visualCapture } = await buildPipeline();
    const state = autonomousBuilderCertification.getState();
    assert.equal(state.engineVersion, "PILLOW-ABC-001");
    assert.equal(state.missionId, "T3-10");
    assert.ok(state.configuration);
    assert.ok(
      AUTONOMOUS_BUILDER_CERTIFICATION_SYSTEM_PATH.includes("AUTONOMOUS_BUILDER_CERTIFICATION"),
    );
    visualCapture.stopCapture();
  });

  test("runCertification validates T3-01 through T3-09", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);

    const report = await engines.autonomousBuilderCertification.runCertification();
    assert.ok(report.certificationReportId.startsWith("abc-report-"));
  ...
  test("end-to-end autonomous builder validation passes when pipeline primed", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
  ...
  test("generates certification report with decision and output path", async () => {
    ...
    const report = await engines.autonomousBuilderCertification.runCertification();
    assert.ok(["pass", "fail", "conditional"].includes(report.finalCertificationDecision));
    assert.ok(report.reportOutputPath);
    assert.ok(report.productionSafetySummary);
  ...
  test("validateForSupervisorSync reports readiness after certification", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    await engines.autonomousBuilderCertification.runCertification();

    const sync = engines.autonomousBuilderCertification.validateForSupervisorSync();
  ...
  test("cockpit snapshot exposes certification status", async () => {
    ...
    const cockpit = engines.autonomousBuilderCertification.getCockpitSnapshot();
    assert.ok(cockpit.totalCertifications >= 1);
    assert.ok(cockpit.missionsPassed >= 0);
    engines.visualCapture.stopCapture();
  });
  ...
  test("getLatestReport returns most recent certification run", async () => {
    ...
    await primeUpstream(engines);
    const report = await engines.autonomousBuilderCertification.runCertification();
    const latest = engines.autonomousBuilderCertification.getLatestReport();
