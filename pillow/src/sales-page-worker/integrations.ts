import { SALES_PAGE_WORKER_IDENTITY } from "./paths.js";
import type {
...
/** Optional live workforce integrations for Q5-08 Sales Page Worker. */
export type SalesPageWorkerDependencies = {
  workerRegistry?: {
  ...
  designWorker?: {
    getDesignReports?: () => Array<{
      designReportId?: string;
      productTitle?: string;
      brandingTheme?: string;
      previewAssets?: Array<{ assetId?: string }>;
    }>;
    getLatestDesignReportId?: () => string | null;
  } | null;
...
export class IntegrationCoordinator {
  ...
  private deps: SalesPageWorkerDependencies = {};

  bind(deps: SalesPageWorkerDependencies = {}) {
    this.deps = { ...deps };
  }
  ...
  enrichFromApprovedProductInformation(input: SalesPageWorkerInput): SalesPageWorkerInput {
    ...
    return {
      ...
      pageType: input.pageType ?? input.productType ?? null,
      productTitle,
  ...
  pullDesignAssetRefs(): string[] {
    const designReports = this.deps.designWorker?.getDesignReports?.() ?? [];
    const refs: string[] = [];
    for (const report of designReports) {
      if (report.designReportId) refs.push(`designReport:${report.designReportId}`);
      for (const asset of report.previewAssets ?? []) {
        if (asset.assetId) refs.push(`designAsset:${asset.assetId}`);
      }
    }
    const latest = this.deps.designWorker?.getLatestDesignReportId?.();
    if (latest && !refs.includes(`designReport:${latest}`)) {
      refs.push(`designReport:${latest}`);
    }
    return [...new Set(refs.filter(Boolean))];
  }

  pullResearchContext(input: SalesPageWorkerInput): {
    enrichment: EnrichmentContext | null;
  } {
    const enriched = this.enrichFromApprovedProductInformation(input);
    const designAssetRefs = this.pullDesignAssetRefs();
    const enrichment: EnrichmentContext | null =
      enriched.researchReportId ||
      enriched.businessId ||
      enriched.factoryMissionId ||
      designAssetRefs.length
        ? {
  ...
  submitReport(pages: SalesPageReport[]): {
    submitted: boolean;
    ...
    if (!primary) {
      return {
        submitted: false,
        executiveReportId: null,
        details: "no_sales_pages_to_submit",
      };
    }
    const result = runtime.submitWorkerReport({
      ...
      missionId: "Q5-08",
      currentStatus: "sales_page_worker_report_prepared",
      progress: Math.round(primary.confidenceScore),
      blockers: pages
        .filter((r) => !r.selfReviewPassed)
        .map((r) => `self_review_blocker:${r.salesPageId}`),
      risks: pages
        .filter((r) => r.researchCompliance === "non_compliant")
        .map((r) => `research_compliance_risk:${r.salesPageId}`),
      evidence: [
        `salesPage:${primary.productTitle}`,
        `type:${primary.pageType}`,
        `headline:${primary.headline.slice(0, 80)}`,
        `sections:${primary.landingPageStructure.length}`,
        `cta:${primary.ctaSummary.slice(0, 80)}`,
        `quality:${primary.qualityReview.slice(0, 120)}`,
      ],
      nextAction:
        "await_pillow_review_of_sales_page_structure_and_copy_no_payments_or_publishing",
      completionStatus: "completed",
      reportType: "worker",
      validated: true,
      salesPageCount: pages.length,
      confidenceScore: primary.confidenceScore,
      neverProcessedPayments: true,
      neverDeliveredProducts: true,
      neverPublishedWebsites: true,
      neverPublishedPagesDirectly: true,
      neverFabricatedTestimonials: true,
    });
    const executiveReportId =
      result?.records?.find((r) => r.reportId)?.reportId ?? `ert-spw-${Date.now()}`;
    appendSpwLog({
      event: "submit_report",
      details: `salesPages=${pages.length} executive=${executiveReportId}`,
    });
    return {
      submitted: true,
      executiveReportId,
  ...
  private provisionWorkerIdentity(workerId: string) {
    const identity = {
      ...
      workerType: SALES_PAGE_WORKER_IDENTITY.workerType,
      ...
      skillProfile: [...SALES_PAGE_WORKER_IDENTITY.skillProfile],
      approvedTools: [...SALES_PAGE_WORKER_IDENTITY.approvedTools],
      authorityLevel: SALES_PAGE_WORKER_IDENTITY.authorityLevel,
    ...
    try {
      this.deps.workerAssignmentEngine?.discoverEligibleWorkers?.({
        missionId: "Q5-08",
        requiredSkills: [...SALES_PAGE_WORKER_IDENTITY.skillProfile],
        validated: true,
      });
    } catch {
      /* assignment optional */
  ...
  private isBound(target: IntegrationTarget): boolean {
    switch (target) {
      ...
      case "design_worker":
        return Boolean(this.deps.designWorker?.getDesignReports);
  ...
  private describe(target: IntegrationTarget, workerId: string, status: string) {
    return `Sales Page Worker ${workerId} ↔ ${target} (${status})`;
  }
}
