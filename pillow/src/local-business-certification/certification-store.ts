...
export function nextReportId() {
  reportSeq += 1;
  return `lbc-rpt-${String(reportSeq).padStart(4, "0")}`;
}

export function resetLbcSequenceForTesting() {
  reportSeq = 0;
}
...
export class CertificationStore {
  private readonly reports = new Map<string, LocalBusinessCertificationReport>();
  ...
  seed(reports: LocalBusinessCertificationReport[]) {
    this.reports.clear();
  ...
  saveReport(report: LocalBusinessCertificationReport, action = "produce_report") {
    this.reports.set(report.reportId, cloneReport(report));
    this.latestReportId = report.reportId;
    this.auditTrail.push({
      timestamp: new Date().toISOString(),
