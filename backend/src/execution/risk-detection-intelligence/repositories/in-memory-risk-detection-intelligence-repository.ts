...
} from "./risk-detection-intelligence-repository.js";

function recordKey(workspaceId: string, recordId: string): string {
  return `${workspaceId}:risk-detection-intelligence:${recordId}`;
}
...
/** In-memory RiskDetectionIntelligenceRepository for Mission 096 tests and local development. */
export class InMemoryRiskDetectionIntelligenceRepository
  implements RiskDetectionIntelligenceRepository
{
  private readonly store = new Map<string, RiskDetectionRecord>();
  ...
  async save(
    ...
    if (existingId) {
      ...
      if (existing) {
        const updated: RiskDetectionRecord = {
          ...existing,
          ...cloned,
    ...
    const record: RiskDetectionRecord = {
      recordId: randomUUID(),
      reportId,
      workspaceId,
      ...cloned,
  ...
  async getById(workspaceId: string, recordId: string): Promise<RiskDetectionRecord | null> {
    const record = this.store.get(recordKey(workspaceId, recordId));
    return record ? structuredClone(record) : null;
  }

  async getByStore(workspaceId: string, storeId: string): Promise<RiskDetectionRecord | null> {
    const recordId = this.storeIndex.get(`${workspaceId}:${storeId}`);
    if (!recordId) return null;
    return this.getById(workspaceId, recordId);
  }

  async list(query: RiskDetectionIntelligenceRepositoryQuery): Promise<RiskDetectionRecord[]> {
...
/** Factory for a fresh in-memory risk detection intelligence repository. */
export function createInMemoryRiskDetectionIntelligenceRepository(): InMemoryRiskDetectionIntelligenceRepository {
  return new InMemoryRiskDetectionIntelligenceRepository();
}
