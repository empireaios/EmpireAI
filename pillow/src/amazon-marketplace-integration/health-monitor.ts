/** R1-02 — Amazon connector health monitoring. */
...
export class HealthMonitor {
  ...
  private lastDecision: AmazonValidationReport["decision"] | null = null;

  recordOperation(decision: AmazonValidationReport["decision"]): void {
  ...
  buildReport(input: {
    ...
    record: AmazonConnectorRecord | null;
    ...
    const notes: string[] = [];
    if (!input.config.enabled) notes.push("Amazon connector disabled by configuration");
    if (input.consecutiveFailures > 0) {
