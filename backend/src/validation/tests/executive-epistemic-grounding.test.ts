...
describe("executive epistemic grounding — Round A deterministic", () => {
  ...
  it("retracts unattested personal retrieval of invented systems", () => {
    const truth = synthTruth();
    const answer =
    ...
    assert.ok(result.violations.includes("UNATTESTED_RETRIEVAL_CLAIM"));
    assert.match(result.message, /Epistemic retraction|cannot substantiate/i);
  });
  ...
  it("temporal precedence: live deploy SHA defeats not-in-production claims", () => {
    ...
    assert.equal(result.adjusted, true);
    assert.ok(result.violations.includes("STALE_OR_FALSE_PRODUCTION_OFFLINE_CLAIM"));
    assert.match(result.message, /Temporal precedence/i);
  });
  ...
  it("integrated truth+epistemic enforcer catches both commerce fabrication and provenance", () => {
    const truth = synthTruth();
    const answer =
  ...
  it("brief includes epistemic discipline and capability registry", () => {
    ...
    const brief = formatExecutiveTruthBriefWithEpistemics(truth, ledger.list());
    ...
    assert.match(formatEpistemicDisciplineBrief({
      truth,
