...
describe("T4-01 Natural UX Conversation", () => {
  beforeEach(() => {
    resetVisualCaptureEngineForTesting();
    resetUiStateMapperForTesting();
    resetComponentRecognitionForTesting();
  ...
  test("buildNaturalUxConversationConfiguration loads defaults", () => {
    const config = buildNaturalUxConversationConfiguration(REPO_ROOT);
    assert.equal(config.enabled, true);
    assert.equal(config.intentRecognitionRulesEnabled, true);
    assert.equal(config.supportedIntentCategories.length, INTENT_CATEGORIES.length);
  });

  test("conversation engine initializes with doctrine doc", async () => {
    const { naturalUxConversation, visualCapture } = await buildPipeline();
    const state = naturalUxConversation.getState();
    assert.equal(state.engineVersion, "PILLOW-NUC-001");
    assert.equal(state.missionId, "T4-01");
    assert.ok(state.configuration);
    assert.ok(NATURAL_UX_CONVERSATION_SYSTEM_PATH.includes("NATURAL_UX_CONVERSATION"));
    visualCapture.stopCapture();
  });

  test("converse recognizes layout intent and produces UX actions", async () => {
    const engines = await buildPipeline();
    const report = engines.naturalUxConversation.converse(
      "Improve the layout spacing on the cockpit dashboard screen",
    );
    assert.ok(report.conversationRunReportId.startsWith("nuc-run-"));
    assert.ok(report.latestTurn);
    assert.equal(report.latestTurn!.intentCategory, "layout_modification");
    assert.ok(report.latestTurn!.generatedUxActions.length > 0);
    assert.ok(report.latestTurn!.confidenceScore > 0);
    engines.visualCapture.stopCapture();
  });

  test("converse produces structured builder requests without executing changes", async () => {
    const engines = await buildPipeline();
    const report = engines.naturalUxConversation.converse(
      "Build a better component button for the settings form",
    );
    const turn = report.latestTurn!;
    assert.ok(turn.generatedBuilderRequests.length > 0);
    for (const req of turn.generatedBuilderRequests) {
      assert.equal(req.forwardedToCertifiedBuilder, false);
    }
    engines.visualCapture.stopCapture();
  });

  test("ambiguous requests trigger clarification", async () => {
    const engines = await buildPipeline();
    const report = engines.naturalUxConversation.converse("fix");
    assert.ok(report.latestTurn);
    assert.ok(
      report.latestTurn!.clarificationStatus === "pending" ||
  ...
  test("conversation preserves session context across turns", async () => {
    const engines = await buildPipeline();
    const first = engines.naturalUxConversation.converse(
      "Improve navigation menu clarity on the cockpit screen",
    );
    const sessionId = first.session.sessionId;
    const second = engines.naturalUxConversation.converse(
      "Also update the theme colors for that screen",
      sessionId,
    );
    assert.equal(second.session.sessionId, sessionId);
    assert.ok(second.session.turns.length >= 2);
    assert.ok(second.latestTurn!.conversationContext.priorTurnCount >= 2);
    engines.visualCapture.stopCapture();
  });

  test("validateForSupervisorSync reports readiness after conversation", async () => {
    const engines = await buildPipeline();
    engines.naturalUxConversation.converse("Analyze the dashboard UX");
    const sync = engines.naturalUxConversation.validateForSupervisorSync();
    assert.ok(sync.readinessScore >= 20);
    assert.ok(["healthy", "degraded", "blocked"].includes(sync.health));
    engines.visualCapture.stopCapture();
  });

  test("cockpit snapshot exposes conversation status", async () => {
    const engines = await buildPipeline();
    engines.naturalUxConversation.converse("Review the table layout on reports screen");
    const cockpit = engines.naturalUxConversation.getCockpitSnapshot();
    assert.ok(cockpit.totalConversations >= 1);
    assert.ok(cockpit.totalTurns >= 1);
    engines.visualCapture.stopCapture();
  });

  test("getLatestReport returns most recent conversation run", async () => {
    const engines = await buildPipeline();
    const report = engines.naturalUxConversation.converse(
      "Explain the current UX scoring approach",
    );
    const latest = engines.naturalUxConversation.getLatestReport();
    assert.equal(latest?.conversationRunReportId, report.conversationRunReportId);
    engines.visualCapture.stopCapture();
  });
});
