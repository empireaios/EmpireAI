/** R1-06 — Walmart connector health monitoring. */
...
export class HealthMonitor {
  ...
  private lastDecision: WalmartValidationReport["decision"] | null = null;

  recordOperation(decision: WalmartValidationReport["decision"]): void {
  ...
  buildReport(input: {
    ...
    record: WalmartConnectorRecord | null;
    ...
    const notes: string[] = [];
    if (!input.config.enabled) notes.push("Walmart connector disabled by configuration");
    if (input.consecutiveFailures > 0) {
      notes.push(`${input.consecutiveFailures} consecutive operation failures`);
    }
    if (input.recoveryAttempts > 0) notes.push(`${input.recoveryAttempts} recovery attempts`);
    if (input.record) {
      notes.push(`Auth: ${input.record.authenticationStatus} · Connection: ${input.record.connectionStatus}`);
    }
