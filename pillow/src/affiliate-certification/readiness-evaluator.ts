...
import { Q8_MISSIONS, precedingMissionIds } from "./mission-catalog.js";
import { AFFILIATE_CERTIFICATION_SYSTEM_PATH } from "./paths.js";
import type {
  ComponentStatusRow,
...
export function evaluateProductionReadiness(
  ...
  return {
    ...
    notes: ready
      ? ["All Q8-01..Q8-08 components observed Completed from repository and runtime evidence"]
...
export function evaluateGovernanceCompliance(
  ...
  const selfPath = join(root, AFFILIATE_CERTIFICATION_SYSTEM_PATH);
  const selfPresent = existsSync(selfPath);
  const selfText = selfPresent ? readFileSync(selfPath, "utf8") : "";
  checks.push({
    missionId: "self",
    governancePath: AFFILIATE_CERTIFICATION_SYSTEM_PATH,
    present: selfPresent,
    containsExpectedLabel: selfText.includes("Affiliate Certification"),
  });
...
export function evaluateWorkflowCompleteness(
  ...
  const stages: WorkflowCompletenessStage[] = Q8_MISSIONS.map((mission) => {
