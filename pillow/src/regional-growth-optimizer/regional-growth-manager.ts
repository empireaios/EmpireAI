/** X4-14 — Regional Growth Optimizer Manager. */
...
import { RegionalPerformanceEngine } from "./regional-performance-engine.js";
import { RegionalRevenueEngine } from "./regional-revenue-engine.js";
import { RegionalProfitabilityEngine } from "./regional-profitability-engine.js";
import { RegionalOpportunityEngine } from "./regional-opportunity-engine.js";
import { RegionalOptimizationEngine } from "./regional-optimization-engine.js";
import { RegionalRecommendationEngine } from "./regional-recommendation-engine.js";
import { RegionalMetadataGenerator } from "./regional-metadata-generator.js";
import { RegionalValidator } from "./regional-validator.js";
import type {
...
export class RegionalGrowthManager {
  private engineRecord: RegionalGrowthOptimizerEngineRecord | null = null;
  ...
  constructor(private readonly deps: RegionalGrowthOptimizerDependencies = {}) {}

  getEngineRecord(): RegionalGrowthOptimizerEngineRecord | null {
  ...
  private dependencyPresence(): RegionalGrowthOptimizerEngineRecord["dependencyPresence"] {
    return {
  ...
  private requireConnected(): RegionalGrowthOptimizerEngineRecord {
    if (!this.engineRecord || this.engineRecord.currentOperationalState === "disconnected") {
      throw new Error(
        "Regional Growth Optimizer not connected — call connectRegionalGrowthOptimizer first",
      );
    }
  ...
  failReport(
    ...
    const engineRecord =
      this.engineRecord ??
      ({
        engineRecordId: "rgo-eng-pending",
        timestamp: new Date().toISOString(),
        engineId: REGIONAL_GROWTH_OPTIMIZER_ID,
        engineVersion: "PILLOW-RGO-001",
        currentOperationalState: "failed",
        healthStatus: "failed",
        validationStatus: "failed",
        supportedCapabilities: [...RGO_CAPABILITIES],
        frameworkModuleId: null,
        dependencyPresence: this.dependencyPresence(),
        metadataVersion: RGO_METADATA_VERSION,
      } satisfies RegionalGrowthOptimizerEngineRecord);
  ...
  registerWithFramework(config: RegionalGrowthOptimizerConfiguration): {
    ...
    const report = this.deps.globalExpansionFramework.registerExpansionModule({
      definition: {
        expansionModuleIdentifier: REGIONAL_GROWTH_OPTIMIZER_ID,
        moduleVersion: RGO_METADATA_VERSION,
    ...
    if (report.validation.decision !== "fail") {
      this.deps.globalExpansionFramework.activateExpansionModule(REGIONAL_GROWTH_OPTIMIZER_ID);
    }

    appendRgoLog({
      event: "framework_registration",
      level: "info",
      details: `Registered Regional Growth Optimizer with GEF: ${report.validation.decision}`,
    });
  ...
  connectRegionalGrowthOptimizer(
    _input: ConnectRegionalGrowthOptimizerInput,
    ...
    this.engineRecord = {
      engineRecordId: `rgo-eng-${Date.now()}`,
      timestamp: new Date().toISOString(),
      engineId: REGIONAL_GROWTH_OPTIMIZER_ID,
      engineVersion: "PILLOW-RGO-001",
    ...
    appendRgoLog({
      ...
      level: "info",
      details:
        "Regional Growth Optimizer connected — structural signals only; never optimize using unvalidated regional intelligence",
    });
  ...
  runDiagnostics(
    ...
    const engineRecord =
      this.engineRecord ??
      ({
        engineRecordId: `rgo-eng-diag-${Date.now()}`,
        timestamp: new Date().toISOString(),
        engineId: REGIONAL_GROWTH_OPTIMIZER_ID,
        engineVersion: "PILLOW-RGO-001",
        currentOperationalState: "connected",
