...
/** Isolated validation DB per test process — avoids parallel test file collisions on disk. */
export function resolveValidationDatabasePath(): string {
  const isolateKey = String(process.pid);
  return `./data/validation/empireai-validation-${isolateKey}.db`;
}
