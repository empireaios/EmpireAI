...
} from "./content-library-repository.js";

function recordKey(workspaceId: string, recordId: string): string {
  return `${workspaceId}:content-library:${recordId}`;
}
...
/** In-memory ContentLibraryRepository for Mission 080 tests and local development. */
export class InMemoryContentLibraryRepository implements ContentLibraryRepository {
  private readonly store = new Map<string, ContentLibraryRecord>();
  ...
  async save(
    ...
    if (existingId) {
      ...
      if (existing) {
        const updated: ContentLibraryRecord = {
          ...existing,
          ...cloned,
  ...
  async list(query: ContentLibraryRepositoryQuery): Promise<ContentLibraryRecord[]> {
...
/** Factory for a fresh in-memory content library repository. */
export function createInMemoryContentLibraryRepository(): InMemoryContentLibraryRepository {
  return new InMemoryContentLibraryRepository();
}
