/** X3-15 — Autonomous Growth Optimizer logging. */

import type { AutonomousGrowthOptimizerConfiguration } from "./configuration.js";
import type { AgoLogEntry } from "./types.js";

const logs: AgoLogEntry[] = [];
...
export function appendAgoLog(input: {
  event: string;
  level: AgoLogEntry["level"];
  details: string;
}): AgoLogEntry {
  const entry: AgoLogEntry = {
    logId: `ago-log-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
...
export function resetAgoLogsForTesting(): void {
  logs.length = 0;
}
