...
export type ClassificationResult = { status: RuntimeCertificationStatus; reason: string };

function repositoryComplete(evidence: RuntimeEvidence): boolean {
  return (
    ...
    evidence.q1014ContractPresent
  );
}

/**
 * Maps observed repository evidence + optional runtime probe to runtime
 * certification status. Decision mapping to overall CertificationDecision
 * (in certification-gates):
...
export function classifyComponent(
  ...
  if (deferred) {
    return {
      ...
      reason: evidence.deferred
        ? "Repository evidence explicitly marks this runtime as deferred"
        : "Input explicitly marked this runtime as deferred",
    };
  }
  ...
  if (!evidence.engineExists) {
    return {
      status: "Blocked",
      reason: "engine.ts not found — runtime implementation blocked",
    };
  }
  ...
  if (complete && probe.reachable) {
    return {
      status: "Certified",
      reason:
        "Engine, config, governance, bridge, test, session, registry, and Q1014 contract (if Q10-13) observed; runtime probe reachable",
    };
  }
  if (complete) {
    return {
      status: "Certified",
      reason:
        "All repository evidence checks passed (engine, config, governance, bridge, test, session, registry, Q1014 contract when applicable)",
    };
  }
  ...
  if (!evidence.q1014ContractPresent) gaps.push("Q1014ConsumableContract not observed in audit-runtime");
  if (probe.reachable === false && probe.evidence !== "No injected runtime handle") {
...
export function classifyMissionDeferred(
  ...
  return Boolean(deferredMissionIds?.includes(runtime.missionId));
}
