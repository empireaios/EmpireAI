/**
 * Integer-minor-unit money model for the Budget Planning Worker (Q9-04).
 *
 * This is a local copy of the cashflow-worker money model, intentionally
 * duplicated rather than imported across worker module boundaries so the
 * Budget Planning Worker stays fully isolated. All budget arithmetic
 * (add/sub/compare/utilisation/variance) operates exclusively on integer
 * minor units (e.g. cents for SGD/USD) via the helpers below. `Math.round`
 * is only ever applied at the single decimal→minor-units parse boundary
 * (`moneyFromDecimal`) when converting a verified planned/actual decimal
 * amount into minor units. No monetary total, utilisation percentage, or
 * variance percentage is ever computed with floating-point multiplication —
 * `moneyToDecimalNumber`/`moneyToDecimalString` exist purely for report
 * display and are never fed back into money math.
 */
...
function assertSameCurrency(a: MoneyMinor, b: MoneyMinor): void {
  if (a.currency !== b.currency) {
    throw new Error(
      `Budget Planning Worker cannot combine mismatched currencies without conversion: ${a.currency} vs ${b.currency}`,
    );
  }
}
...
/**
 * Parse a verified planned/actual decimal amount (major units, e.g. 10.50)
 * into integer minor units (1050). This is the ONLY boundary where a
 * rounding operation on a decimal value is permitted. String inputs are
 * parsed digit-by-digit to avoid floating-point representation error
 * entirely; number inputs use a single `Math.round` at this boundary only.
 */
 ...
 * multiplication of the two money amounts.
 */
