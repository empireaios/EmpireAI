...
describe("T5-07 Continuous UX Evolution", () => {
  beforeEach(() => {
    resetVisualCaptureEngineForTesting();
    resetUiStateMapperForTesting();
    resetComponentRecognitionForTesting();
  ...
  test("buildContinuousUxEvolutionConfiguration loads defaults", () => {
    const config = buildContinuousUxEvolutionConfiguration(REPO_ROOT);
    assert.equal(config.enabled, true);
    assert.equal(config.recommendOnlyMode, true);
    assert.equal(config.recommendationRulesEnabled, true);
    assert.ok(EVOLUTION_CATEGORIES.includes("layout_evolution"));
  });

  test("continuous UX evolution initializes with doctrine doc", async () => {
    const engines = await buildPipeline();
    const state = engines.continuousUxEvolution.getState();
    assert.equal(state.engineVersion, "PILLOW-CUE-001");
    assert.equal(state.missionId, "T5-07");
    assert.ok(CONTINUOUS_UX_EVOLUTION_SYSTEM_PATH.includes("CONTINUOUS_UX_EVOLUTION"));
    engines.visualCapture.stopCapture();
  });

  test("optimize produces machine-readable UX evolution records", async () => {
    const engines = await buildPipeline();
    ...
    assert.ok(report.evolutionRunReportId.startsWith("cue-run-"));
    assert.ok(report.records.length > 0, `expected records, got ${report.records.length}`);
    for (const record of report.records) {
      assert.equal(record.recommendOnly, true);
      assert.ok(record.uxEvolutionId.startsWith("cue-"));
      assert.ok(record.metadataVersion);
      assert.ok(record.expectedUxBenefit);
      assert.ok(record.recommendedUxImprovements.length > 0);
    }
    engines.visualCapture.stopCapture();
  });

  test("identifies workflow improvements from T5 chain", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.continuousUxEvolution.optimize();
    assert.ok(
      report.records.some(
        (r) =>
          r.evolutionCategory === "workflow_evolution" ||
          r.evolutionCategory === "productivity_evolution" ||
          r.sourceWorkflowEvolutionId !== null,
      ),
    );
    engines.visualCapture.stopCapture();
  });

  test("generates navigation evolution recommendations", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.continuousUxEvolution.optimize();
    assert.ok(
      report.records.some(
        (r) =>
          r.evolutionCategory === "navigation_evolution" ||
          r.evolutionCategory === "dashboard_evolution" ||
          r.recommendedUxImprovements.some((i) => i.toLowerCase().includes("nav")),
      ) ||
        report.records.some((r) => r.evolutionCategory === "layout_evolution"),
    );
    engines.visualCapture.stopCapture();
  });

  test("generates accessibility evolution recommendations", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.continuousUxEvolution.optimize();
    assert.ok(
      report.records.some(
        (r) =>
          r.evolutionCategory === "accessibility_evolution" ||
          r.evolutionCategory === "visual_consistency_evolution" ||
          r.evolutionCategory === "component_evolution" ||
          r.evolutionCategory === "user_experience_evolution",
      ),
    );
    engines.visualCapture.stopCapture();
  });

  test("never applies UX changes automatically (recommend-only)", async () => {
    const engines = await buildPipeline();
    const report = engines.continuousUxEvolution.optimize();
    for (const record of report.records) {
      assert.equal(record.recommendOnly, true);
  ...
  test("start and stop continuous evolution toggles monitoring state", async () => {
    const engines = await buildPipeline();
    const started = engines.continuousUxEvolution.startContinuousEvolution();
    assert.equal(started.health.continuousEvolutionActive, true);
    const stopped = engines.continuousUxEvolution.stopContinuousEvolution();
    assert.equal(stopped.health.continuousEvolutionActive, false);
    engines.visualCapture.stopCapture();
  });

  test("validateForSupervisorSync reports readiness after evolution", async () => {
    const engines = await buildPipeline();
    engines.continuousUxEvolution.optimize();
    const sync = engines.continuousUxEvolution.validateForSupervisorSync();
    assert.ok(sync.readinessScore >= 20);
    assert.ok(["healthy", "degraded", "blocked"].includes(sync.health));
    engines.visualCapture.stopCapture();
  });

  test("cockpit snapshot exposes evolution health", async () => {
    const engines = await buildPipeline();
    engines.continuousUxEvolution.optimize();
    const cockpit = engines.continuousUxEvolution.getCockpitSnapshot();
    assert.ok(cockpit.totalEvolutionCycles >= 1);
    assert.ok(cockpit.engineStatus);
    engines.visualCapture.stopCapture();
  });

  test("getLatestReport returns most recent evolution run", async () => {
    const engines = await buildPipeline();
    const report = engines.continuousUxEvolution.optimize();
    const latest = engines.continuousUxEvolution.getLatestReport();
    assert.equal(latest?.evolutionRunReportId, report.evolutionRunReportId);
    engines.visualCapture.stopCapture();
  });

  test("maintains evolution history after optimization", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.continuousUxEvolution.optimize();
    assert.ok(report.historyEntries.length > 0);
    const history = engines.continuousUxEvolution.getEvolutionHistory();
    assert.ok(history.length > 0);
    assert.ok(history[0]!.historyId.startsWith("cue-history-"));
    engines.visualCapture.stopCapture();
  });
});
