import { CASHFLOW_WORKER_IDENTITY } from "./paths.js";
...
export type CashflowWorkerDependencies = {
  ...
  /** Q9-02 Accounting Worker — dependency injection only, the sole source of verified cash records. */
  accountingWorker?: {
    getReports?: () => unknown[];
    getEntries?: () => InjectedAccountingEntry[];
    getAccounts?: () => InjectedLedgerAccount[];
    getAssets?: () => unknown[];
    getLiabilities?: () => unknown[];
...
export class IntegrationCoordinator {
  ...
  private deps: CashflowWorkerDependencies = {};

  bind(deps: CashflowWorkerDependencies = {}) {
    this.deps = { ...deps };
  }
  ...
  /** Fetch verified accounting entries from the injected Accounting Worker — never fabricated. */
  ...
  fetchAccountingAccounts(): InjectedLedgerAccount[] {
    try {
      return this.deps.accountingWorker?.getAccounts?.() ?? [];
    } catch {
      return [];
    }
  }
  ...
  recordAudit(report: CashflowReport): { audited: boolean; details: string } {
  ...
  recordMemory(report: CashflowReport): void {
    try {
  ...
  submitReport(report: CashflowReport): {
    submitted: boolean;
  ...
  private registerIdentity() {
    const identity = {
      workerId: CASHFLOW_WORKER_IDENTITY.workerId,
      workerName: CASHFLOW_WORKER_IDENTITY.workerName,
      workerType: CASHFLOW_WORKER_IDENTITY.workerType,
      ...
      skillProfile: [...CASHFLOW_WORKER_IDENTITY.skillProfile],
      approvedTools: [...CASHFLOW_WORKER_IDENTITY.approvedTools],
      authorityLevel: CASHFLOW_WORKER_IDENTITY.authorityLevel,
      missionId: "Q9-03",
