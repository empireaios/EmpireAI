...
} from "./mobile-messaging-repository.js";

function recordKey(workspaceId: string, recordId: string): string {
  return `${workspaceId}:mobile-messaging:${recordId}`;
}
...
/** In-memory MobileMessagingRepository for Mission 085 tests and local development. */
export class InMemoryMobileMessagingRepository implements MobileMessagingRepository {
  private readonly store = new Map<string, MobileMessagingRecord>();
  ...
  async save(
    ...
    if (existingId) {
      ...
      if (existing) {
        const updated: MobileMessagingRecord = {
          ...existing,
          ...cloned,
    ...
    const record: MobileMessagingRecord = {
      recordId: randomUUID(),
      blueprintId,
      workspaceId,
      ...cloned,
  ...
  async getById(workspaceId: string, recordId: string): Promise<MobileMessagingRecord | null> {
    const record = this.store.get(recordKey(workspaceId, recordId));
    return record ? structuredClone(record) : null;
  }

  async getByStore(workspaceId: string, storeId: string): Promise<MobileMessagingRecord | null> {
    const recordId = this.storeIndex.get(`${workspaceId}:${storeId}`);
    if (!recordId) return null;
    return this.getById(workspaceId, recordId);
  }

  async list(query: MobileMessagingRepositoryQuery): Promise<MobileMessagingRecord[]> {
...
/** Factory for a fresh in-memory mobile messaging repository. */
export function createInMemoryMobileMessagingRepository(): InMemoryMobileMessagingRepository {
  return new InMemoryMobileMessagingRepository();
}
