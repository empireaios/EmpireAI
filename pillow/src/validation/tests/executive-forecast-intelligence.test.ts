...
describe("E3-12 Executive Forecast Intelligence", () => {
  test("buildFallbackExecutiveForecastIntelligence returns constitutional forecast model", () => {
    const view = buildFallbackExecutiveForecastIntelligence();
    assert.equal(view.engineVersion, "E3-12");
    assert.equal(view.executiveForecastPipeline.length, EXECUTIVE_FORECAST_PIPELINE.length);
    assert.deepEqual(view.forecastPrinciples, [...EXECUTIVE_FORECAST_PRINCIPLES]);
    assert.equal(view.governedDomains.length, GOVERNED_FORECAST_DOMAINS.length);
    assert.ok(view.executiveForecasts.length >= 10);
    assert.ok(view.revenueForecast.length >= 4);
    assert.ok(view.forecastAccuracy.length >= 5);
    assert.ok(view.recommendedActions.length >= 1);
    assert.equal(view.readyForE313, true);
  });

  test("assembleExecutiveForecastIntelligence integrates E3-01 through E3-11 E2-01", () => {
    ...
    const view = assembleExecutiveForecastIntelligence({
      executiveFinanceFramework,
      capitalAllocationEngine,
      executiveBudgetPlanner,
      investmentEvaluationEngine,
    ...
    assert.ok(
      view.executiveForecasts.every(
        (f) => f.forecastId && f.projectedRevenue && f.evidence.length >= 1,
      ),
    );
    assert.equal(view.readyForE313, true);
  });
