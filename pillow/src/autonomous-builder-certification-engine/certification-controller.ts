/** T3-10 — Autonomous Builder Certification orchestration controller. */

import { appendCertificationLog } from "./certification-logging.js";
import { HealthMonitor } from "./health-monitor.js";
import { RecoveryManager } from "./recovery-manager.js";
import { AutonomousBuilderCertificationManager } from "./autonomous-builder-certification-manager.js";
import { T3CapabilityValidator, type T3EngineBundle } from "./t3-capability-validator.js";
import { EndToEndAutonomousBuilderTestRunner } from "./end-to-end-autonomous-builder-test-runner.js";
...
export class CertificationController {
  private config: AutonomousBuilderCertificationConfiguration;
  private status: CertificationStatus = "idle";
  private latestReport: AutonomousBuilderCertificationReport | null = null;
  ...
  private readonly sessionManager = new AutonomousBuilderCertificationManager();
  private readonly capabilityValidator = new T3CapabilityValidator();
  private readonly e2eRunner = new EndToEndAutonomousBuilderTestRunner();
  ...
  constructor(
    ...
    private engines: T3EngineBundle,
  ...
  getConfiguration(): AutonomousBuilderCertificationConfiguration {
    return { ...this.config };
  }

  updateConfiguration(config: AutonomousBuilderCertificationConfiguration): void {
    this.config = config;
  }
  ...
  async runCertification(): Promise<AutonomousBuilderCertificationReport> {
    if (!this.config.enabled) {
      throw new Error("Autonomous Builder Certification is disabled by configuration");
    }
    ...
    appendCertificationLog({
      ...
      level: "info",
      details: "Autonomous Builder certification started",
    });
