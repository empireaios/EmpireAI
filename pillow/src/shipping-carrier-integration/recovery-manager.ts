/** R2-11 — Shipping carrier automatic recovery. */
...
export class RecoveryManager {
  ...
  recordFailure(error: string, config: ShippingCarrierIntegrationConfiguration): boolean {
    this.consecutiveFailures += 1;
    appendSciLog({
      event: "carrier_failure",
      level: "warn",
    ...
    appendSciLog({
      ...
      level: "info",
      details: `Carrier recovery attempt ${this.recoveryAttempts}`,
    });
