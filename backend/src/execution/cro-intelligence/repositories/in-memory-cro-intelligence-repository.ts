...
} from "./cro-intelligence-repository.js";

function recordKey(workspaceId: string, recordId: string): string {
  return `${workspaceId}:cro-intelligence:${recordId}`;
}
...
/** In-memory CroIntelligenceRepository for Mission 083 tests and local development. */
export class InMemoryCroIntelligenceRepository implements CroIntelligenceRepository {
  private readonly store = new Map<string, CroIntelligenceRecord>();
  ...
  async save(
    ...
    if (existingId) {
      ...
      if (existing) {
        const updated: CroIntelligenceRecord = {
          ...existing,
          ...cloned,
    ...
    const record: CroIntelligenceRecord = {
      recordId: randomUUID(),
      reportId,
      workspaceId,
      ...cloned,
  ...
  async getById(workspaceId: string, recordId: string): Promise<CroIntelligenceRecord | null> {
    const record = this.store.get(recordKey(workspaceId, recordId));
    return record ? structuredClone(record) : null;
  }

  async getByStore(workspaceId: string, storeId: string): Promise<CroIntelligenceRecord | null> {
    const recordId = this.storeIndex.get(`${workspaceId}:${storeId}`);
    if (!recordId) return null;
    return this.getById(workspaceId, recordId);
  }

  async list(query: CroIntelligenceRepositoryQuery): Promise<CroIntelligenceRecord[]> {
...
/** Factory for a fresh in-memory CRO intelligence repository. */
export function createInMemoryCroIntelligenceRepository(): InMemoryCroIntelligenceRepository {
  return new InMemoryCroIntelligenceRepository();
}
