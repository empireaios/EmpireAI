...
} from "./pricing-intelligence-repository.js";

function recordKey(workspaceId: string, recordId: string): string {
  return `${workspaceId}:pricing-intelligence:${recordId}`;
}
...
/** In-memory PricingIntelligenceRepository for Mission 086 tests and local development. */
export class InMemoryPricingIntelligenceRepository implements PricingIntelligenceRepository {
  private readonly store = new Map<string, PricingIntelligenceRecord>();
  ...
  async save(
    ...
    if (existingId) {
      ...
      if (existing) {
        const updated: PricingIntelligenceRecord = {
          ...existing,
          ...cloned,
    ...
    const record: PricingIntelligenceRecord = {
      recordId: randomUUID(),
      blueprintId,
      workspaceId,
      ...cloned,
  ...
  async getById(workspaceId: string, recordId: string): Promise<PricingIntelligenceRecord | null> {
    const record = this.store.get(recordKey(workspaceId, recordId));
    return record ? structuredClone(record) : null;
  }

  async getByStore(workspaceId: string, storeId: string): Promise<PricingIntelligenceRecord | null> {
    const recordId = this.storeIndex.get(`${workspaceId}:${storeId}`);
    if (!recordId) return null;
    return this.getById(workspaceId, recordId);
  }

  async list(query: PricingIntelligenceRepositoryQuery): Promise<PricingIntelligenceRecord[]> {
...
/** Factory for a fresh in-memory pricing intelligence repository. */
export function createInMemoryPricingIntelligenceRepository(): InMemoryPricingIntelligenceRepository {
  return new InMemoryPricingIntelligenceRepository();
}
