...
} from "./persistent-memory-intelligence-repository.js";

function recordKey(workspaceId: string, recordId: string): string {
  return `${workspaceId}:persistent-memory-intelligence:${recordId}`;
}
...
/** In-memory PersistentMemoryIntelligenceRepository for Mission 093 tests and local development. */
export class InMemoryPersistentMemoryIntelligenceRepository
  implements PersistentMemoryIntelligenceRepository
{
  private readonly store = new Map<string, PersistentMemoryRecord>();
  ...
  async save(
    ...
    if (existingId) {
      ...
      if (existing) {
        const updated: PersistentMemoryRecord = {
          ...existing,
          ...cloned,
  ...
  async list(
    query: PersistentMemoryIntelligenceRepositoryQuery,
  ): Promise<PersistentMemoryRecord[]> {
...
/** Factory for a fresh in-memory persistent memory intelligence repository. */
export function createInMemoryPersistentMemoryIntelligenceRepository(): InMemoryPersistentMemoryIntelligenceRepository {
  return new InMemoryPersistentMemoryIntelligenceRepository();
}
