...
export type QueueRuntimeDependencies = {
...
export class QrtIntegrationCoordinator {
  private deps: QueueRuntimeDependencies = {};

  bind(deps: QueueRuntimeDependencies = {}) {
  ...
  notifyDispatch(dispatch: unknown) {
    const por = this.deps.pillowOrchestrationRuntime;
    if (por?.invokeWorker) {
      return por.invokeWorker({ type: "queue_dispatch_signal", dispatch });
    }
    return { notified: false, offline: true };
  }

  notifyMissionRuntime(dispatch: unknown) {
    const mr = this.deps.missionRuntime;
    if (mr?.monitor) {
      return mr.monitor({ type: "queue_dispatch_signal", dispatch });
    }
    return { notified: false, offline: true };
  }
