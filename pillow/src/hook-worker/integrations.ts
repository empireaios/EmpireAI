import { HOOK_WORKER_IDENTITY } from "./paths.js";
import type {
  IntegrationHandshake,
  IntegrationTarget,
  HookReport,
  HookWorkerInput,
} from "./types.js";
import { appendHkwLog } from "./hkw-logging.js";

/** Optional live workforce integrations for Q4-06 Hook Worker. */
export type HookWorkerDependencies = {
  workerRegistry?: {
    registerWorker: (input: Record<string, unknown>) => unknown;
  } | null;
  workerLifecycle?: {
    createWorker: (input: Record<string, unknown>) => unknown;
    activateWorker: (input: Record<string, unknown>) => unknown;
  } | null;
  workerAssignmentEngine?: {
    discoverEligibleWorkers: (input: Record<string, unknown>) => unknown;
  } | null;
  scriptWorker?: {
    getLatestScriptId?: () => string | null;
    getScripts?: () => Array<{
      scriptId?: string;
      channelId?: string;
      topicId?: string;
      contentFormat?: string;
      scriptTitle?: string;
      scriptSections?: Array<{
        sectionId?: string;
        sectionType?: string;
        heading?: string;
        narration?: string;
      }>;
      narrationReadyText?: string;
      targetAudience?: string;
    }>;
  } | null;
  executiveReportingRuntime?: {
    submitWorkerReport: (input: Record<string, unknown>) => {
      records?: Array<{ reportId?: string }>;
      engineRecord?: { lastReportType?: string | null } | null;
    };
  } | null;
  workerPerformanceReview?: {
    registerPerformanceWorker: (input: Record<string, unknown>) => unknown;
  } | null;
  workerRecoverySystem?: {
    registerRecoverableWorker: (input: Record<string, unknown>) => unknown;
  } | null;
};

export class IntegrationCoordinator {
  private handshakes: IntegrationHandshake[] = [];
  private deps: HookWorkerDependencies = {};

  bind(deps: HookWorkerDependencies = {}) {
    this.deps = { ...deps };
  }

  getHandshakes() {
    return this.handshakes.map((h) => ({ ...h }));
  }

  connect(workerId: string, targets: string[]): IntegrationHandshake[] {
    const now = new Date().toISOString();
    const resolved: IntegrationHandshake[] = [];
    for (const target of targets as IntegrationTarget[]) {
      const status = this.isBound(target) ? "bound" : "ready";
      const handshake: IntegrationHandshake = {
        target,
        status,
        details: this.describe(target, workerId, status),
        timestamp: now,
      };
      resolved.push(handshake);
      appendHkwLog({
        event: "integration_handshake",
        details: `${target}:${status}`,
      });
    }
    this.handshakes = resolved;
    this.provisionWorkerIdentity(workerId);
    return this.getHandshakes();
  }

  enrichFromScriptWorker(input: HookWorkerInput): HookWorkerInput {
    const scripts = this.deps.scriptWorker?.getScripts?.() ?? [];
    const scriptId = input.scriptId ?? this.deps.scriptWorker?.getLatestScriptId?.() ?? null;
    const scriptMatch =
      (scriptId ? scripts.find((s) => s.scriptId === scriptId) : null) ??
      (scripts.length ? scripts[scripts.length - 1] : null);
    return {
      ...input,
      scriptId: input.scriptId ?? scriptMatch?.scriptId ?? scriptId ?? null,
      channelId: input.channelId ?? scriptMatch?.channelId ?? null,
      topicId: input.topicId ?? scriptMatch?.topicId ?? null,
      contentFormat: input.contentFormat ?? scriptMatch?.contentFormat ?? null,
      scriptTitle: input.scriptTitle ?? scriptMatch?.scriptTitle ?? null,
      scriptSections:
        input.scriptSections ??
        scriptMatch?.scriptSections?.map((s) => ({
          sectionId: s.sectionId ?? "",
          sectionType: s.sectionType ?? "body",
          heading: s.heading ?? "",
          narration: s.narration ?? "",
        })) ??
        null,
      narrationReadyText: input.narrationReadyText ?? scriptMatch?.narrationReadyText ?? null,
      targetAudience: input.targetAudience ?? scriptMatch?.targetAudience ?? null,
    };
  }

  submitReport(reports: HookReport[]): {
    submitted: boolean;
    executiveReportId: string | null;
    details: string;
  } {
    const runtime = this.deps.executiveReportingRuntime;
    if (!runtime?.submitWorkerReport) {
      return {
        submitted: false,
        executiveReportId: null,
        details: "executive_reporting_runtime_unavailable",
      };
    }
    const primary = reports[reports.length - 1];
    if (!primary) {
      return {
        submitted: false,
        executiveReportId: null,
        details: "no_hook_reports_to_submit",
      };
    }
    const result = runtime.submitWorkerReport({
      reportingEntity: primary.workerId,
      entityType: "worker",
      businessId: primary.channelId,
      missionId: "Q4-06",
      currentStatus: "hook_optimized",
      progress: Math.round(primary.confidenceScore),
      blockers: reports
        .filter((r) => !r.selfReviewPassed)
        .map((r) => `self_review_blocker:${r.hookReportId}:failed`),
      risks: [],
      evidence: [
        `primaryHook:${primary.primaryHook.hookType}`,
        ...primary.alternativeHooks.map((h) => `altHook:${h.hookType}`),
        ...primary.curiosityGaps.map((g) => `gap:${g.gapId}`),
      ],
      nextAction: "await_visual_video_workers_no_direct_production",
      completionStatus: "completed",
      reportType: "worker",
      validated: true,
      hookReportCount: reports.length,
      contentFormat: primary.contentFormat,
      neverRewroteCompleteScript: true,
      neverGeneratedThumbnails: true,
    });
    const executiveReportId =
      result?.records?.find((r) => r.reportId)?.reportId ?? `ert-hkw-${Date.now()}`;
    appendHkwLog({
      event: "submit_report",
      details: `hookReports=${reports.length} executive=${executiveReportId}`,
    });
    return {
      submitted: true,
      executiveReportId,
      details: "submitted_to_executive_reporting_runtime",
    };
  }

  private provisionWorkerIdentity(workerId: string) {
    const identity = {
      workerId,
      workerName: HOOK_WORKER_IDENTITY.workerName,
      workerType: HOOK_WORKER_IDENTITY.workerType,
      department: HOOK_WORKER_IDENTITY.department,
      factory: HOOK_WORKER_IDENTITY.factory,
      role: HOOK_WORKER_IDENTITY.role,
      reportingLine: [...HOOK_WORKER_IDENTITY.reportingLine],
      skillProfile: [...HOOK_WORKER_IDENTITY.skillProfile],
      approvedTools: [...HOOK_WORKER_IDENTITY.approvedTools],
      authorityLevel: HOOK_WORKER_IDENTITY.authorityLevel,
      certificationStatus: "certified",
      operationalStatus: "active",
      validated: true,
    };
    try {
      this.deps.workerRegistry?.registerWorker?.(identity);
    } catch {
      /* registry may reject duplicates */
    }
    try {
      this.deps.workerLifecycle?.createWorker?.(identity);
      this.deps.workerLifecycle?.activateWorker?.({ workerId, validated: true });
    } catch {
      /* lifecycle optional */
    }
    try {
      this.deps.workerAssignmentEngine?.discoverEligibleWorkers?.({
        missionId: "Q4-06",
        requiredSkills: [...HOOK_WORKER_IDENTITY.skillProfile],
        validated: true,
      });
    } catch {
      /* assignment optional */
    }
    try {
      this.deps.workerPerformanceReview?.registerPerformanceWorker?.({
        workerId,
        validated: true,
      });
    } catch {
      /* performance optional */
    }
    try {
      this.deps.workerRecoverySystem?.registerRecoverableWorker?.({
        workerId,
        validated: true,
      });
    } catch {
      /* recovery optional */
    }
  }

  private isBound(target: IntegrationTarget): boolean {
    switch (target) {
      case "worker_registry":
        return Boolean(this.deps.workerRegistry?.registerWorker);
      case "worker_lifecycle":
        return Boolean(this.deps.workerLifecycle?.createWorker);
      case "worker_assignment_engine":
        return Boolean(this.deps.workerAssignmentEngine?.discoverEligibleWorkers);
      case "script_worker":
        return Boolean(this.deps.scriptWorker?.getScripts);
      case "executive_reporting_runtime":
        return Boolean(this.deps.executiveReportingRuntime?.submitWorkerReport);
      case "worker_performance_review":
        return Boolean(this.deps.workerPerformanceReview?.registerPerformanceWorker);
      case "worker_recovery_system":
        return Boolean(this.deps.workerRecoverySystem?.registerRecoverableWorker);
      default:
        return false;
    }
  }

  private describe(target: IntegrationTarget, workerId: string, status: string) {
    return `Hook Worker ${workerId} ↔ ${target} (${status})`;
  }
}
