/** R1-08 — eBay connector health monitoring. */
...
export class HealthMonitor {
  ...
  private lastDecision: EbayValidationReport["decision"] | null = null;

  recordOperation(decision: EbayValidationReport["decision"]): void {
  ...
  buildReport(input: {
    ...
    record: EbayConnectorRecord | null;
    ...
    const notes: string[] = [];
    if (!input.config.enabled) notes.push("eBay connector disabled by configuration");
    if (input.consecutiveFailures > 0) {
