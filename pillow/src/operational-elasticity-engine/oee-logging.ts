/** X3-11 — Operational Elasticity Engine logging. */

import type { OperationalElasticityEngineConfiguration } from "./configuration.js";
import type { OeeLogEntry } from "./types.js";

const logs: OeeLogEntry[] = [];
...
export function appendOeeLog(input: {
  event: string;
  level: OeeLogEntry["level"];
  details: string;
}): OeeLogEntry {
  const entry: OeeLogEntry = {
    logId: `oee-log-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
...
export function resetOeeLogsForTesting(): void {
  logs.length = 0;
}
