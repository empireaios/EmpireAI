...
describe("T3-04 Theme Builder", () => {
  ...
  test("buildThemeBuilderConfiguration loads defaults", () => {
    const config = buildThemeBuilderConfiguration(REPO_ROOT);
    assert.equal(config.enabled, true);
    assert.equal(config.minConfidenceThreshold, 0.4);
    assert.equal(config.allowedThemeScopes.length, THEME_SCOPES.length);
  });

  test("theme builder initializes with doctrine doc", async () => {
    const { themeBuilder, visualCapture } = await buildPipeline();
    const state = themeBuilder.getState();
    assert.equal(state.engineVersion, "PILLOW-TB-001");
    assert.equal(state.missionId, "T3-04");
    assert.ok(THEME_BUILDER_SYSTEM_PATH.includes("THEME_BUILDER"));
    visualCapture.stopCapture();
  });

  test("generateThemes produces machine-readable theme records", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);

    const report = engines.themeBuilder.generateThemes();
    assert.ok(report.themeGenerationReportId.startsWith("tb-report-"));
    assert.ok(["pass", "partial", "fail"].includes(report.validation.decision));

    engines.visualCapture.stopCapture();
  });

  test("theme records include color, typography, spacing, and interaction tokens", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);

    const report = engines.themeBuilder.generateThemes();
    for (const record of report.records) {
      assert.ok(record.themeId.startsWith("tb-record-"));
      assert.ok(record.colorTokens.length > 0);
      assert.ok(record.typographyTokens.length > 0);
      assert.ok(record.spacingTokens.length > 0);
      assert.ok(record.interactionStateTokens.length > 0);
      assert.ok(record.generatedThemeCode.length > 0);
  ...
  test("safety checks enforce frontend-only scope", async () => {
    ...
    const report = engines.themeBuilder.generateThemes();
    for (const record of report.records) {
      if (record.themeStatus === "blocked") continue;
      ...
      for (const target of record.targetFiles) {
        assert.ok(target.startsWith("empireai-web/"), `Expected frontend path: ${target}`);
      }
    }
    engines.visualCapture.stopCapture();
  });
  ...
  test("validateForSupervisorSync reports readiness after generation", async () => {
    ...
    const sync = engines.themeBuilder.validateForSupervisorSync();
  ...
  test("cockpit snapshot exposes builder status", async () => {
    ...
    const cockpit = engines.themeBuilder.getCockpitSnapshot();
  ...
  test("handles missing upstream data gracefully", async () => {
    const { themeBuilder, visualCapture } = await buildPipeline();
    const report = themeBuilder.generateThemes();
    assert.ok(report);
