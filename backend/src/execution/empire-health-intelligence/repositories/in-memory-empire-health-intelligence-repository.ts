...
} from "./empire-health-intelligence-repository.js";

function recordKey(workspaceId: string, recordId: string): string {
  return `${workspaceId}:empire-health-intelligence:${recordId}`;
}
...
/** In-memory EmpireHealthIntelligenceRepository for Mission 095 tests and local development. */
export class InMemoryEmpireHealthIntelligenceRepository implements EmpireHealthIntelligenceRepository {
  private readonly store = new Map<string, EmpireHealthRecord>();
  ...
  async save(
    ...
    if (existingId) {
      ...
      if (existing) {
        const updated: EmpireHealthRecord = {
          ...existing,
          ...cloned,
    ...
    const record: EmpireHealthRecord = {
      recordId: randomUUID(),
      reportId,
      workspaceId,
      ...cloned,
  ...
  async getById(workspaceId: string, recordId: string): Promise<EmpireHealthRecord | null> {
    const record = this.store.get(recordKey(workspaceId, recordId));
    return record ? structuredClone(record) : null;
  }

  async getByStore(workspaceId: string, storeId: string): Promise<EmpireHealthRecord | null> {
    const recordId = this.storeIndex.get(`${workspaceId}:${storeId}`);
    if (!recordId) return null;
    return this.getById(workspaceId, recordId);
  }

  async list(query: EmpireHealthIntelligenceRepositoryQuery): Promise<EmpireHealthRecord[]> {
...
/** Factory for a fresh in-memory empire health intelligence repository. */
export function createInMemoryEmpireHealthIntelligenceRepository(): InMemoryEmpireHealthIntelligenceRepository {
  return new InMemoryEmpireHealthIntelligenceRepository();
}
