/** T1-03 — Recognition health monitoring. */
...
export class HealthMonitor {
  private lastSuccessfulRecognitionAt: string | null = null;
  ...
  recordRecognition(durationMs: number, success: boolean): void {
    if (success) {
      this.lastSuccessfulRecognitionAt = new Date().toISOString();
  ...
  buildReport(input: {
    ...
    const recognitionsPerMinute =
      uptimeMs > 0
        ? Math.round((input.performance.successfulRecognitions / uptimeMs) * 60000)
        : 0;
    ...
    if (input.backlogSize >= input.config.resultBufferLimit) healthScore -= 10;
    ...
    const notes: string[] = [];
    if (!input.config.enabled) notes.push("Recognition disabled by configuration");
    ...
    return {
      ...
      recognitionEnabled: input.config.enabled,
      isRecognizing: input.status === "recognizing",
      lastSuccessfulRecognitionAt: this.lastSuccessfulRecognitionAt,
      consecutiveFailures: input.consecutiveFailures,
      recoveryAttempts: input.recoveryAttempts,
      averageProcessingDurationMs: avgDuration,
      recognitionsPerMinute,
