/** R1-01 — Marketplace Connector Framework logging. */
...
const SENSITIVE_PATTERN = /(token|secret|password|api[_-]?key|authorization|bearer)/i;
...
export function appendFrameworkLog(input: {
  ...
  const entry: FrameworkLogEntry = {
    logId: `mcf-log-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
