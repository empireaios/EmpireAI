...
export function resetSeowSequenceForTesting() {
  reportSeq = 0;
  sessionSeq = 0;
  assetSeq = 0;
}
...
export function nextReportId() {
  reportSeq += 1;
  return `seow-rpt-${String(reportSeq).padStart(4, "0")}`;
}

export function nextSessionId() {
  sessionSeq += 1;
  return `seow-sess-${String(sessionSeq).padStart(4, "0")}`;
}

export function nextAssetId(kind: string) {
  assetSeq += 1;
  return `seow-${kind}-${String(assetSeq).padStart(4, "0")}`;
}

export class SeoStore {
  private sessions = new Map<string, SeoSession>();
  private reports = new Map<string, SeoContentReport>();
  private versionHistory: ContentVersionEntry[] = [];
  private latestSessionId: string | null = null;
  private latestReportId: string | null = null;
  private auditTrail: Array<{ timestamp: string; action: string; detail: string }> = [];

  seed(reports: SeoContentReport[]) {
  ...
  saveSession(session: SeoSession) {
    this.sessions.set(session.sessionId, structuredClone(session));
    this.latestSessionId = session.sessionId;
    this.audit(`save_session:${session.sessionId}`, `topic=${session.topic}`);
  }
  ...
  saveReport(report: SeoContentReport) {
    this.reports.set(report.reportId, structuredClone(report));
    this.latestReportId = report.reportId;
    this.audit(`save_report:${report.reportId}`, `topic=${report.topic}`);
  }
  ...
  appendVersion(entry: ContentVersionEntry) {
    this.versionHistory.push({ ...entry });
  }
