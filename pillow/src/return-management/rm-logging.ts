/** R2-13 — Return management logging. */

import type { ReturnManagementConfiguration } from "./configuration.js";
import type { ReturnLogEntry } from "./types.js";

const logs: ReturnLogEntry[] = [];
...
export function appendRmLog(input: {
  event: string;
  level: ReturnLogEntry["level"];
  details: string;
}): ReturnLogEntry {
  const entry: ReturnLogEntry = {
...
export function resetRmLogsForTesting(): void {
  logs.length = 0;
}
