...
describe("T5-04 Productivity Intelligence", () => {
  beforeEach(() => {
    resetVisualCaptureEngineForTesting();
    resetUiStateMapperForTesting();
    resetComponentRecognitionForTesting();
  ...
  test("buildProductivityIntelligenceConfiguration loads defaults", () => {
    const config = buildProductivityIntelligenceConfiguration(REPO_ROOT);
    assert.equal(config.enabled, true);
    assert.equal(config.learnOnlyMode, true);
    assert.equal(config.patternDetectionRulesEnabled, true);
    assert.ok(PRODUCTIVITY_CATEGORIES.includes("workflow_pattern"));
  });

  test("productivity intelligence initializes with doctrine doc", async () => {
    const engines = await buildPipeline();
    const state = engines.productivityIntelligence.getState();
    assert.equal(state.engineVersion, "PILLOW-PIE-001");
    assert.equal(state.missionId, "T5-04");
    assert.ok(PRODUCTIVITY_INTELLIGENCE_SYSTEM_PATH.includes("PRODUCTIVITY_INTELLIGENCE"));
    engines.visualCapture.stopCapture();
  });

  test("learn produces machine-readable productivity intelligence records", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.productivityIntelligence.learn();
    assert.ok(report.learningRunReportId.startsWith("pie-run-"));
    assert.notEqual(report.validation.decision, "fail");
    for (const record of report.records) {
      assert.equal(record.learnOnly, true);
      assert.ok(record.productivityId.startsWith("pie-"));
      assert.ok(record.metadataVersion);
      assert.ok(record.workflowPatternSummary);
      assert.ok(record.navigationPatternSummary);
      assert.ok(record.taskSequenceSummary);
      assert.ok(record.bottleneckSummary);
    }
    engines.visualCapture.stopCapture();
  });

  test("learns workflow bottleneck patterns from T5 chain", async () => {
    const engines = await buildPipeline();
    engines.continuousScreenObservation.observe({
      uiSnapshot: {
    ...
    assert.ok(
      report.records.some((r) =>
        r.productivityObservations.includes("workflow_bottleneck") ||
        r.productivityObservations.includes("time_utilization"),
      ),
    );
    engines.visualCapture.stopCapture();
  });

  test("learns task repetition patterns from interaction tracking", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.productivityIntelligence.learn();
    assert.ok(
      report.records.some((r) => r.productivityObservations.includes("task_repetition")),
    );
    engines.visualCapture.stopCapture();
  });

  test("never applies workflow changes automatically (learn-only)", async () => {
    const engines = await buildPipeline();
    const report = engines.productivityIntelligence.learn();
    for (const record of report.records) {
      assert.equal(record.learnOnly, true);
  ...
  test("start and stop continuous learning toggles monitoring state", async () => {
    const engines = await buildPipeline();
    const started = engines.productivityIntelligence.startContinuousLearning();
    assert.equal(started.health.continuousLearningActive, true);
    const stopped = engines.productivityIntelligence.stopContinuousLearning();
  ...
  test("validateForSupervisorSync reports readiness after learning", async () => {
    ...
    const sync = engines.productivityIntelligence.validateForSupervisorSync();
  ...
  test("cockpit snapshot exposes learning health", async () => {
    const engines = await buildPipeline();
    engines.productivityIntelligence.learn();
    const cockpit = engines.productivityIntelligence.getCockpitSnapshot();
  ...
  test("getLatestReport returns most recent learning run", async () => {
    const engines = await buildPipeline();
    const report = engines.productivityIntelligence.learn();
    const latest = engines.productivityIntelligence.getLatestReport();
