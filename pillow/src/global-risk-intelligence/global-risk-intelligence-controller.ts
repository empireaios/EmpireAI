/** X4-14 — Global Risk Intelligence orchestration controller. */

import { appendRgoLog } from "./gri-logging.js";
import { HealthMonitor } from "./health-monitor.js";
import { RecoveryManager } from "./recovery-manager.js";
import { RegionalGrowthManager } from "./global-risk-intelligence-manager.js";
...
export class RegionalGrowthController {
  private config: GlobalRiskIntelligenceConfiguration;
  ...
  initialize(): void {
    ...
    appendRgoLog({
      ...
      level: "info",
      details:
        "Global Risk Intelligence ready — structural signals only; never optimize using unvalidated regional intelligence",
    });
  }
  ...
  getConfiguration(): GlobalRiskIntelligenceConfiguration {
    return { ...this.config };
  }

  updateConfiguration(config: GlobalRiskIntelligenceConfiguration): void {
    this.config = config;
  }
  ...
  connectGlobalRiskIntelligence(input: ConnectGlobalRiskIntelligenceInput = {}): RgoRunReport {
    if (!this.config.enabled) throw new Error("Global Risk Intelligence is disabled");
    this.status = "connecting";
    const report = this.manager.connectGlobalRiskIntelligence(input, this.config);
