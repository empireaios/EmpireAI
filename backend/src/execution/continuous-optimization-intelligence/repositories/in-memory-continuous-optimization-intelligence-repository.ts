...
} from "./continuous-optimization-intelligence-repository.js";

function recordKey(workspaceId: string, recordId: string): string {
  return `${workspaceId}:continuous-optimization-intelligence:${recordId}`;
}
...
/** In-memory ContinuousOptimizationIntelligenceRepository for Mission 094 tests and local development. */
export class InMemoryContinuousOptimizationIntelligenceRepository
  implements ContinuousOptimizationIntelligenceRepository
{
  private readonly store = new Map<string, ContinuousOptimizationRecord>();
  ...
  async save(
    ...
    if (existingId) {
      ...
      if (existing) {
        const updated: ContinuousOptimizationRecord = {
          ...existing,
          ...cloned,
    ...
    const record: ContinuousOptimizationRecord = {
      recordId: randomUUID(),
      reportId,
      workspaceId,
      ...cloned,
...
/** Factory for a fresh in-memory continuous optimization intelligence repository. */
export function createInMemoryContinuousOptimizationIntelligenceRepository(): InMemoryContinuousOptimizationIntelligenceRepository {
  return new InMemoryContinuousOptimizationIntelligenceRepository();
}
