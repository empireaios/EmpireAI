/** R4-19 — Certification validator. */

import { CERTIFICATION_SCHEMA_VERSION, COC_METADATA_VERSION } from "./paths.js";
...
export class CertificationValidator {
  validateCertificationResult(input: {
    ...
    if (!input.config.requiredValidationRulesEnabled) {
      return {
        validationReportId: `coc-val-${Date.now()}`,
  ...
  validateReportIntegrity(
    report: CustomerOperationsCertificationReport,
  ): CertificationValidationReport {
    const started = Date.now();
    const errors: string[] = [];
    const warnings: string[] = [];

    if (!report.certificationId.startsWith("coc-run-")) {
    ...
    if (report.metadataVersion !== COC_METADATA_VERSION) {
