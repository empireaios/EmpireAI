import { DESIGN_WORKER_IDENTITY } from "./paths.js";
import type {
  DesignWorkerInput,
  DesignWorkerReport,
...
/** Optional live workforce integrations for Q5-07 Design Worker. */
export type DesignWorkerDependencies = {
  workerRegistry?: {
...
export class IntegrationCoordinator {
  ...
  private deps: DesignWorkerDependencies = {};

  bind(deps: DesignWorkerDependencies = {}) {
    this.deps = { ...deps };
  }
  ...
  enrichFromApprovedProductInformation(input: DesignWorkerInput): DesignWorkerInput {
    ...
    const productTitle =
      ...
      (missionMatch?.businessName ? `${missionMatch.businessName} Design Pack` : null);
  ...
  pullResearchContext(input: DesignWorkerInput): {
  ...
  submitReport(reports: DesignWorkerReport[]): {
    submitted: boolean;
    ...
    if (!primary) {
      return {
        submitted: false,
        executiveReportId: null,
        details: "no_design_reports_to_submit",
      };
    }
    const result = runtime.submitWorkerReport({
      ...
      missionId: "Q5-07",
      currentStatus: "design_worker_report_prepared",
      progress: Math.round(primary.confidenceScore),
      blockers: reports
        .filter((r) => !r.selfReviewPassed)
        .map((r) => `self_review_blocker:${r.designReportId}`),
      risks: reports
        .filter((r) => r.researchCompliance === "non_compliant")
        .map((r) => `research_compliance_risk:${r.designReportId}`),
      evidence: [
        `designReport:${primary.productTitle}`,
        `type:${primary.productType}`,
        `theme:${primary.brandingTheme}`,
        `assets:${primary.allAssets.length}`,
        `exports:${primary.exportFormats.join(",")}`,
        `quality:${primary.qualityReview.slice(0, 120)}`,
      ],
      nextAction: "await_pillow_review_of_export_ready_design_assets_no_publish_or_delivery",
      completionStatus: "completed",
      reportType: "worker",
      validated: true,
      designReportCount: reports.length,
      confidenceScore: primary.confidenceScore,
      neverBuiltSalesPages: true,
      neverProcessedPayments: true,
      neverDeliveredProducts: true,
      neverPublishedAssetsDirectly: true,
      neverPublishedProductsDirectly: true,
    });
    const executiveReportId =
      result?.records?.find((r) => r.reportId)?.reportId ?? `ert-dw-${Date.now()}`;
    appendDwLog({
      event: "submit_report",
      details: `designReports=${reports.length} executive=${executiveReportId}`,
    });
    return {
      submitted: true,
      executiveReportId,
  ...
  private provisionWorkerIdentity(workerId: string) {
    const identity = {
      ...
      workerType: DESIGN_WORKER_IDENTITY.workerType,
      ...
      skillProfile: [...DESIGN_WORKER_IDENTITY.skillProfile],
      approvedTools: [...DESIGN_WORKER_IDENTITY.approvedTools],
      authorityLevel: DESIGN_WORKER_IDENTITY.authorityLevel,
    ...
    try {
      this.deps.workerAssignmentEngine?.discoverEligibleWorkers?.({
        missionId: "Q5-07",
        requiredSkills: [...DESIGN_WORKER_IDENTITY.skillProfile],
        validated: true,
      });
    } catch {
      /* assignment optional */
  ...
  private describe(target: IntegrationTarget, workerId: string, status: string) {
    return `Design Worker ${workerId} ↔ ${target} (${status})`;
  }
}
