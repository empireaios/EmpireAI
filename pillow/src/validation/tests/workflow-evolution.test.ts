...
describe("T5-05 Workflow Evolution", () => {
  beforeEach(() => {
    resetVisualCaptureEngineForTesting();
    resetUiStateMapperForTesting();
    resetComponentRecognitionForTesting();
  ...
  test("buildWorkflowEvolutionConfiguration loads defaults", () => {
    const config = buildWorkflowEvolutionConfiguration(REPO_ROOT);
    assert.equal(config.enabled, true);
    assert.equal(config.recommendOnlyMode, true);
    assert.equal(config.frictionDetectionRulesEnabled, true);
    assert.ok(EVOLUTION_CATEGORIES.includes("workflow_simplification"));
  });

  test("workflow evolution initializes with doctrine doc", async () => {
    const engines = await buildPipeline();
    const state = engines.workflowEvolution.getState();
    assert.equal(state.engineVersion, "PILLOW-WFE-001");
    assert.equal(state.missionId, "T5-05");
    assert.ok(WORKFLOW_EVOLUTION_SYSTEM_PATH.includes("WORKFLOW_EVOLUTION"));
    engines.visualCapture.stopCapture();
  });

  test("evolve produces machine-readable workflow evolution records", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.workflowEvolution.evolve();
    assert.ok(report.evolutionRunReportId.startsWith("wfe-run-"));
    assert.notEqual(report.validation.decision, "fail");
    for (const record of report.records) {
      assert.equal(record.recommendOnly, true);
      assert.ok(record.workflowEvolutionId.startsWith("wfe-"));
      assert.ok(record.metadataVersion);
      assert.ok(record.workflowFrictionSummary);
      assert.ok(record.recommendedWorkflowImprovements.length > 0);
      assert.ok(record.estimatedProductivityBenefit);
    }
    engines.visualCapture.stopCapture();
  });

  test("detects workflow friction from productivity intelligence chain", async () => {
    const engines = await buildPipeline();
    engines.continuousScreenObservation.observe({
      uiSnapshot: {
    ...
    assert.ok(
      report.records.some(
        (r) =>
          r.evolutionCategory === "workflow_acceleration" ||
          r.evolutionCategory === "process_optimization" ||
          r.evolutionCategory === "user_effort_reduction",
      ),
    );
    engines.visualCapture.stopCapture();
  });

  test("identifies workflow simplification from task repetition", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.workflowEvolution.evolve();
    assert.ok(
      report.records.some((r) => r.evolutionCategory === "task_reduction"),
    );
    engines.visualCapture.stopCapture();
  });

  test("never applies workflow changes automatically (recommend-only)", async () => {
    const engines = await buildPipeline();
    const report = engines.workflowEvolution.evolve();
    for (const record of report.records) {
      assert.equal(record.recommendOnly, true);
  ...
  test("start and stop continuous evolution toggles monitoring state", async () => {
    const engines = await buildPipeline();
    const started = engines.workflowEvolution.startContinuousEvolution();
    assert.equal(started.health.continuousEvolutionActive, true);
    const stopped = engines.workflowEvolution.stopContinuousEvolution();
  ...
  test("validateForSupervisorSync reports readiness after evolution", async () => {
    ...
    const sync = engines.workflowEvolution.validateForSupervisorSync();
  ...
  test("cockpit snapshot exposes evolution health", async () => {
    const engines = await buildPipeline();
    engines.workflowEvolution.evolve();
    const cockpit = engines.workflowEvolution.getCockpitSnapshot();
  ...
  test("getLatestReport returns most recent evolution run", async () => {
    const engines = await buildPipeline();
    const report = engines.workflowEvolution.evolve();
    const latest = engines.workflowEvolution.getLatestReport();
