/** X1-12 — Growth Initialization Engine logging. */
...
export function appendGieLog(input: {
  event: string;
  level: GrowthLogEntry["level"];
  details: string;
}): GrowthLogEntry {
  const entry: GrowthLogEntry = {
    logId: `gie-log-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
...
export function resetGieLogsForTesting(): void {
  logs.length = 0;
}
