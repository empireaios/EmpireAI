...
| Candidate | Draft, unmerged PR #5, branch `fix/pillow-integrated-closure-20260920`; code commit `53e370ed7f793e845ad4e65b4b7cf8ec843c859a` repaired the two `9046a3d4` CI failures and passed [Product](https://github.com/empireaios/EmpireAI/actions/runs/36238300749), [Semantic](https://github.com/empireaios/EmpireAI/actions/runs/36238300758) and [Runtime](https://github.com/empireaios/EmpireAI/actions/runs/36238300753) on the exact head. Inspect the branch ref for later commits. |
| Fully verified candidate | `53e370ed7f793e845ad4e65b4b7cf8ec843c859a` has all three hosted offline CI passes. This is an engineering proof, not a hosted restart, safe data migration, Birth, production promotion or commerce approval. |
...
Production Railway variable names include `DATABASE_PATH` and `RAILWAY_VOLUME_MOUNT_PATH`, but neither `SHADOW_CEO_DATA_DIR` nor `EMPIRE_DATA_DIR`. Current Shadow CEO code defaults its database, request-owner JSON and authority directory to `cwd/.data` rather than the attached `/data` volume. The actual on-disk state of that directory is **unknown**. Preserve it and require a quiesced migration/backup readback before deploying a path change; do not initialize an empty authority store as proof of continuity.

## Next executable actions

1. Repair the Shadow CEO store root so Railway requires the attached volume, and make request-owner JSON writes atomic and corrupt reads fail closed. Verify locally and on all three hosted workflows. Preserve old `.data` state before any production cutover. The independent-process Amazon cursor proof already passed Product on [intermediate `d05f728`](https://github.com/empireaios/EmpireAI/actions/runs/36091201021), with fake transport only.
