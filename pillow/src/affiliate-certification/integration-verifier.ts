...
const SPECIAL_BIND_EXPANSIONS: Record<string, string[]> = {
  bindsQ801ThroughQ807: Q8_MISSIONS.filter((m) => m.missionId !== "Q8-08").map(
    (m) => m.dependencyKey,
  ),
};
...
/**
 * Verifies integration wiring among Q8-01..Q8-08 workers purely from
...
export function verifyIntegrations(
  ...
  const rows: IntegrationCheckRow[] = Q8_MISSIONS.map((mission) => {
