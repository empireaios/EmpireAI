/** X3-06 — Supplier Scale Engine health monitoring. */
...
export class HealthMonitor {
  ...
  buildReport(input: {
    ...
    record: SupplierScaleEngineRecord | null;
    ...
    const notes: string[] = [];
    if (!input.config.enabled) notes.push("Supplier Scale Engine disabled");
    if (input.consecutiveFailures > 0) {
    ...
    notes.push(
      "Never recommend supplier expansion without validated capacity — structural signals only",
    );
