...
describe("T5-02 Autonomous UX Audit", () => {
  beforeEach(() => {
    resetVisualCaptureEngineForTesting();
    resetUiStateMapperForTesting();
    resetComponentRecognitionForTesting();
  ...
  test("buildAutonomousUxAuditConfiguration loads defaults", () => {
    const config = buildAutonomousUxAuditConfiguration(REPO_ROOT);
    assert.equal(config.enabled, true);
    assert.equal(config.auditOnlyMode, true);
    assert.equal(config.issueDetectionRulesEnabled, true);
    assert.ok(UX_ISSUE_CATEGORIES.includes("accessibility_issue"));
  });

  test("autonomous UX audit initializes with doctrine doc", async () => {
    const engines = await buildPipeline();
    const state = engines.autonomousUxAudit.getState();
    assert.equal(state.engineVersion, "PILLOW-AUA-001");
    assert.equal(state.missionId, "T5-02");
    assert.ok(AUTONOMOUS_UX_AUDIT_SYSTEM_PATH.includes("AUTONOMOUS_UX_AUDIT"));
    engines.visualCapture.stopCapture();
  });

  test("audit produces machine-readable UX audit record", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.autonomousUxAudit.audit();
    assert.ok(report.auditRunReportId.startsWith("aua-"));
    assert.equal(report.audit.auditOnly, true);
    assert.ok(report.audit.metadataVersion);
    assert.ok(typeof report.audit.confidenceScore === "number");
    assert.notEqual(report.validation.decision, "fail");
    engines.visualCapture.stopCapture();
  });

  test("detects loading state issues from observation", async () => {
    const engines = await buildPipeline();
    engines.continuousScreenObservation.observe({
      uiSnapshot: {
    ...
    const report = engines.autonomousUxAudit.audit();
    assert.ok(
      report.audit.detectedUxIssues.some((i) => i.category === "loading_state_issue"),
    );
    engines.visualCapture.stopCapture();
  });

  test("detects issues from primed T2 upstream engines", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.autonomousUxAudit.audit();
    assert.ok(report.audit.detectedUxIssues.length >= 0);
    assert.ok(report.audit.evidenceReferences.length >= 0);
    engines.visualCapture.stopCapture();
  });

  test("never applies UX changes automatically (audit-only)", async () => {
    const engines = await buildPipeline();
    const report = engines.autonomousUxAudit.audit();
    assert.equal(report.audit.auditOnly, true);
    assert.equal(
      report.validation.errors.some((e) => e.includes("audit-only") === false && e.length > 0),
      false,
    );
    engines.visualCapture.stopCapture();
  });

  test("start and stop continuous audit toggles monitoring state", async () => {
    const engines = await buildPipeline();
    const started = engines.autonomousUxAudit.startContinuousAudit();
    assert.equal(started.health.continuousAuditActive, true);
    const stopped = engines.autonomousUxAudit.stopContinuousAudit();
    assert.equal(stopped.health.continuousAuditActive, false);
    engines.visualCapture.stopCapture();
  });

  test("validateForSupervisorSync reports readiness after audit", async () => {
    const engines = await buildPipeline();
    engines.autonomousUxAudit.audit();
    const sync = engines.autonomousUxAudit.validateForSupervisorSync();
    assert.ok(sync.readinessScore >= 20);
    assert.ok(["healthy", "degraded", "blocked"].includes(sync.health));
    engines.visualCapture.stopCapture();
  });

  test("cockpit snapshot exposes audit health", async () => {
    const engines = await buildPipeline();
    engines.autonomousUxAudit.audit();
    const cockpit = engines.autonomousUxAudit.getCockpitSnapshot();
    assert.ok(cockpit.totalAudits >= 1);
    assert.ok(cockpit.engineStatus);
    engines.visualCapture.stopCapture();
  });

  test("getLatestReport returns most recent audit run", async () => {
    const engines = await buildPipeline();
    const report = engines.autonomousUxAudit.audit();
    const latest = engines.autonomousUxAudit.getLatestReport();
    assert.equal(latest?.auditRunReportId, report.auditRunReportId);
    engines.visualCapture.stopCapture();
  });
});
