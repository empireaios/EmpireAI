/** R3-16 — Dashboard health monitoring. */
...
export class HealthMonitor {
  ...
  buildReport(input: {
    ...
    record: ExecutiveFinancialDashboardRecord | null;
    ...
    const notes: string[] = [];
    if (!input.config.enabled) notes.push("Executive financial dashboard disabled");
    if (input.consecutiveFailures > 0) {
      notes.push(`${input.consecutiveFailures} consecutive operation failures`);
    }
    if (input.recoveryAttempts > 0) notes.push(`${input.recoveryAttempts} recovery attempts`);
    notes.push(`${input.totalSnapshots} dashboard snapshot(s) tracked`);
