/** X3-01 — Autonomous Scaling Framework health monitoring. */
...
export class HealthMonitor {
  ...
  private lastDecision: ScalingValidationReport["decision"] | null = null;

  recordOperation(decision: ScalingValidationReport["decision"]): void {
  ...
  buildReport(input: {
    ...
    modules: AutonomousScalingFrameworkRecord[];
    ...
    const notes: string[] = [];
    if (!input.config.enabled) notes.push("Autonomous Scaling Framework disabled by configuration");
    if (input.consecutiveFailures > 0) {
