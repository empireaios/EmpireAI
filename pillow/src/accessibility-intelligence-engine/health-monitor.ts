/** T2-06 — Accessibility Intelligence health monitoring. */

import type { AccessibilityIntelligenceConfiguration } from "./configuration.js";
import type {
  AccessibilityHealthReport,
  ...
  buildReport(input: {
    ...
    const avgDuration =
      this.reviewDurations.length > 0
        ? Math.round(
            this.reviewDurations.reduce((a, b) => a + b, 0) /
              this.reviewDurations.length,
          )
        : 0;
    ...
    const notes: string[] = [];
    if (!input.config.enabled) notes.push("Accessibility Intelligence disabled by configuration");
    if (input.reviewsCompleted === 0) notes.push("No accessibility reviews completed yet");
    if (input.consecutiveFailures > 0) {
