/** R2-20 — Certification validator. */

import { CERTIFICATION_SCHEMA_VERSION, SOC_METADATA_VERSION } from "./paths.js";
...
export class CertificationValidator {
  validateCertificationResult(input: {
    ...
    if (!input.config.requiredValidationRulesEnabled) {
      return {
        validationReportId: `soc-val-${Date.now()}`,
  ...
  validateReportIntegrity(report: SupplierOperationsCertificationReport): CertificationValidationReport {
    const started = Date.now();
    const errors: string[] = [];
    const warnings: string[] = [];

    if (!report.certificationId.startsWith("soc-run-")) {
    ...
    if (report.metadataVersion !== SOC_METADATA_VERSION) {
