/** X2-21 — Portfolio Certified health monitoring. */
...
export class HealthMonitor {
  ...
  buildReport(input: {
    ...
    const notes: string[] = [];
    if (!input.config.enabled) notes.push("Portfolio Certified disabled");
    if (input.consecutiveFailures > 0) {
      notes.push(`${input.consecutiveFailures} consecutive operation failures`);
    }
    if (input.recoveryAttempts > 0) notes.push(`${input.recoveryAttempts} recovery attempts`);
    notes.push(`Certification reports: ${input.totalCertificationReports}`);
    notes.push("Safe test mode — production systems are not modified");
