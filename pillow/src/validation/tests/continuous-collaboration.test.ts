...
describe("T4-09 Continuous Collaboration", () => {
  beforeEach(() => {
    resetVisualCaptureEngineForTesting();
    resetUiStateMapperForTesting();
    resetComponentRecognitionForTesting();
  ...
  test("buildContinuousCollaborationConfiguration loads defaults", () => {
    const config = buildContinuousCollaborationConfiguration(REPO_ROOT);
    assert.equal(config.enabled, true);
    assert.equal(config.sessionPersistenceRulesEnabled, true);
    assert.equal(config.preferenceApplicationRulesEnabled, true);
  });

  test("continuous collaboration initializes with doctrine doc", async () => {
    const engines = await buildPipeline();
    const state = engines.continuousCollaboration.getState();
    assert.equal(state.engineVersion, "PILLOW-CC-001");
    assert.equal(state.missionId, "T4-09");
    assert.ok(CONTINUOUS_COLLABORATION_SYSTEM_PATH.includes("CONTINUOUS_COLLABORATION"));
    engines.visualCapture.stopCapture();
  });

  test("synchronize maintains persistent collaboration session", async () => {
    const engines = await buildPipeline();
    engines.multiProposalGenerator.generateProposals();
    engines.approvalWorkflow.submitApproval({ approvalDecision: "defer" });
    const report = engines.continuousCollaboration.synchronize();
    assert.ok(report.collaborationRunReportId.startsWith("cc-run-"));
    assert.ok(report.session.collaborationSessionId.startsWith("cc-session-"));
    assert.notEqual(report.validation.decision, "fail");
    engines.visualCapture.stopCapture();
  });

  test("tracks pending proposals and approvals", async () => {
    const engines = await buildPipeline();
    engines.multiProposalGenerator.generateProposals();
    engines.approvalWorkflow.present({});
    const report = engines.continuousCollaboration.synchronize();
    assert.ok(report.session.pendingProposalIds.length > 0);
    assert.ok(report.session.pendingApprovalIds.length > 0);
    engines.visualCapture.stopCapture();
  });

  test("applies learned preferences advisorially", async () => {
    const engines = await buildPipeline();
    engines.multiProposalGenerator.generateProposals();
    engines.approvalWorkflow.submitApproval({ approvalDecision: "approve" });
    engines.preferenceLearning.learn();
    const report = engines.continuousCollaboration.synchronize({ applyPreferences: true });
    for (const pref of report.session.appliedCollaborationPreferences) {
      assert.equal(pref.explicitOverrideAllowed, true);
    }
    engines.visualCapture.stopCapture();
  });

  test("collaboration record is machine-readable with metadata", async () => {
    const engines = await buildPipeline();
    engines.multiProposalGenerator.generateProposals();
    const report = engines.continuousCollaboration.synchronize();
    const session = report.session;
    assert.ok(session.metadataVersion);
    assert.ok(session.collaborationContextSummary);
    assert.ok(typeof session.confidenceScore === "number");
    engines.visualCapture.stopCapture();
  });

  test("never auto-approves or auto-executes UX changes", async () => {
    const engines = await buildPipeline();
    engines.multiProposalGenerator.generateProposals();
    engines.approvalWorkflow.submitApproval({ approvalDecision: "approve" });
    const report = engines.continuousCollaboration.synchronize();
    assert.equal(
      report.validation.errors.some((e) => e.includes("approve UX changes automatically")),
      false,
    );
    assert.equal(
      report.validation.errors.some((e) => e.includes("execute UX changes automatically")),
      false,
    );
    engines.visualCapture.stopCapture();
  });

  test("validateForSupervisorSync reports readiness after synchronization", async () => {
    const engines = await buildPipeline();
    engines.multiProposalGenerator.generateProposals();
    engines.continuousCollaboration.synchronize();
    const sync = engines.continuousCollaboration.validateForSupervisorSync();
    assert.ok(sync.readinessScore >= 20);
    assert.ok(["healthy", "degraded", "blocked"].includes(sync.health));
    engines.visualCapture.stopCapture();
  });

  test("context restoration preserves collaboration across sessions", async () => {
    const engines = await buildPipeline();
    engines.multiProposalGenerator.generateProposals();
    const first = engines.continuousCollaboration.synchronize();
    engines.continuousCollaboration.endSession(first.session.collaborationSessionId);
    const second = engines.continuousCollaboration.synchronize({ restoreContext: true });
    assert.ok(second.session.collaborationContextSummary);
    engines.visualCapture.stopCapture();
  });

  test("getLatestReport returns most recent synchronization run", async () => {
    const engines = await buildPipeline();
    engines.multiProposalGenerator.generateProposals();
    const report = engines.continuousCollaboration.synchronize();
    const latest = engines.continuousCollaboration.getLatestReport();
    assert.equal(latest?.collaborationRunReportId, report.collaborationRunReportId);
    engines.visualCapture.stopCapture();
  });
});
