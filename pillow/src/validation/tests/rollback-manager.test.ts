...
async function buildPipeline() {
  ...
  const visualFoundation = createVisualFoundationCertificationEngine(
    ...
    {
      configuration: {
        reportOutputRoot: ".pillow-rollback-manager-test",
...
describe("T3-08 Rollback Manager", () => {
  beforeEach(() => {
    resetVisualCaptureEngineForTesting();
    resetUiStateMapperForTesting();
    resetComponentRecognitionForTesting();
  ...
  test("buildRollbackManagerConfiguration loads defaults", () => {
    const config = buildRollbackManagerConfiguration(REPO_ROOT);
    assert.equal(config.enabled, true);
    assert.equal(config.restorePointRulesEnabled, true);
    assert.equal(config.allowedRollbackScopes.length, ROLLBACK_SCOPES.length);
    assert.ok(config.rollbackTriggerRules.length >= ROLLBACK_TRIGGERS.length - 1);
  });

  test("rollback manager initializes with doctrine doc", async () => {
    const { rollbackManager, visualCapture } = await buildPipeline();
    const state = rollbackManager.getState();
    assert.equal(state.engineVersion, "PILLOW-RM-001");
    assert.equal(state.missionId, "T3-08");
    assert.ok(ROLLBACK_MANAGER_SYSTEM_PATH.includes("ROLLBACK_MANAGER"));
    visualCapture.stopCapture();
  });

  test("createRestorePoint produces safe restore point", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);

    const point = engines.rollbackManager.createRestorePoint();
    assert.ok(point.restorePointId.startsWith("rm-restore-"));
    assert.equal(point.restorePointStatus, "active");
    assert.ok(point.fileSnapshotReferences.length > 0);
    engines.visualCapture.stopCapture();
  });

  test("executeRollback produces machine-readable rollback reports", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    engines.rollbackManager.createRestorePoint();

    const report = engines.rollbackManager.executeRollback("manual_rollback_request");
    assert.ok(report.rollbackRunReportId.startsWith("rm-run-"));
    assert.ok(["pass", "partial", "fail", "blocked"].includes(report.validation.decision));
    assert.ok(report.restorePoints.length > 0);
    engines.visualCapture.stopCapture();
  });

  test("rollback reports include verification metadata", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    engines.rollbackManager.createRestorePoint();
    const report = engines.rollbackManager.executeRollback("manual_rollback_request");

    for (const rbReport of report.reports) {
      assert.ok(rbReport.rollbackReportId.startsWith("rm-report-"));
      assert.ok(ROLLBACK_TRIGGERS.includes(rbReport.rollbackTrigger));
      assert.ok(rbReport.rollbackVerificationResult);
    }
    engines.visualCapture.stopCapture();
  });

  test("validateForSupervisorSync reports readiness after rollback", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    engines.rollbackManager.createRestorePoint();
    engines.rollbackManager.executeRollback("manual_rollback_request");

    const sync = engines.rollbackManager.validateForSupervisorSync();
    assert.ok(sync.readinessScore >= 20);
    assert.ok(["healthy", "degraded", "blocked"].includes(sync.health));
    engines.visualCapture.stopCapture();
  });

  test("cockpit snapshot exposes rollback status", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    engines.rollbackManager.createRestorePoint();
    engines.rollbackManager.executeRollback("manual_rollback_request");

    const cockpit = engines.rollbackManager.getCockpitSnapshot();
    assert.ok(cockpit.totalRollbacks >= 0);
    engines.visualCapture.stopCapture();
  });

  test("handles missing upstream data gracefully", async () => {
    const { rollbackManager, visualCapture } = await buildPipeline();
    const point = rollbackManager.createRestorePoint();
    assert.ok(point.restorePointId);
    visualCapture.stopCapture();
  });

  test("isRollbackVerified reflects verification result", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    engines.rollbackManager.createRestorePoint();
    engines.rollbackManager.executeRollback("manual_rollback_request");

    const verified = engines.rollbackManager.isRollbackVerified();
    const report = engines.rollbackManager.getLatestReport();
    const expected =
      report?.reports.every((r) => r.rollbackVerificationResult.verified) ?? false;
    assert.equal(verified, expected);
    engines.visualCapture.stopCapture();
  });
});
