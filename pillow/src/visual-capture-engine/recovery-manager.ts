/** T1-01 — Automatic capture recovery. */
...
export class RecoveryManager {
  ...
  recordFailure(error: string, config: VisualCaptureConfiguration): boolean {
  ...
  shouldRetry(config: VisualCaptureConfiguration): boolean {
    return config.autoRecover && this.recoveryAttempts <= config.maxRetryAttempts;
  }
