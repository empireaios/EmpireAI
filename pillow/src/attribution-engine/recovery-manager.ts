/** R5-09 — Attribution Engine recovery manager. */
...
export class RecoveryManager {
  ...
  recordFailure(error: string, config: AttributionEngineConfiguration): boolean {
    this.consecutiveFailures += 1;
    appendAttLog({
      event: "attribution_failures",
      level: "warn",
    ...
    appendAttLog({
      ...
      level: "info",
      details: `Attribution Engine recovery attempt ${this.recoveryAttempts}`,
    });
