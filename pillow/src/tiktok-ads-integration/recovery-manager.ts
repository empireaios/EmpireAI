/** R5-04 — TikTok Ads recovery manager. */
...
export class RecoveryManager {
  ...
  recordFailure(error: string, config: TikTokAdsIntegrationConfiguration): boolean {
    this.consecutiveFailures += 1;
    appendTaiLog({
      event: "tiktok_ads_failure",
      level: "warn",
    ...
    appendTaiLog({
      ...
      level: "info",
      details: `TikTok Ads recovery attempt ${this.recoveryAttempts}`,
    });
