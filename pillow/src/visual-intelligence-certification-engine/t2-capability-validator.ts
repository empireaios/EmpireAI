/** T5-10 — T2 UX Intelligence programme validator. */
...
export class T2CapabilityValidator {
  async validate(
    engine: UxIntelligenceCertificationEngine,
    ...
    appendCertificationLog({
      ...
      level: "info",
      details: "Validating T2 UX Intelligence",
    });

    const result: ProgrammeValidationResult = {
      programmeId: "T2",
      programmeName: "UX Intelligence",
      passed: false,
    ...
    try {
      ...
      if (config.runNestedProgrammeCertifications) {
        ...
        if (report.finalCertificationDecision === "fail") {
          result.errors.push("T2 nested certification failed");
        }
      } else {
        ...
        if (latest) {
          ...
          result.warnings.push("No prior T2 certification report — using supervisor sync only");
        }
      }

      if (config.validateHealthReporting && state.health.status === "failed") {
        result.errors.push("T2 health reporting indicates failed status");
      }
      if (supervisor.readinessScore < config.requiredPassThreshold) {
        result.warnings.push(
          `T2 readiness ${supervisor.readinessScore} below threshold ${config.requiredPassThreshold}`,
        );
      }
      ...
      result.errors.push(error instanceof Error ? error.message : "T2 validation failed");
    }
    ...
    appendCertificationLog({
      ...
      details: `T2 ${result.passed ? "PASS" : "FAIL"} · ${result.durationMs}ms`,
    });
    return result;
  }
}
