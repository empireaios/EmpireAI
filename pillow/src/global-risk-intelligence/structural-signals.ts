...
export function computeStructuralRegionalSignals(
  ...
  const optimizationTraceId = `gri-trace-${hashScore(seed, 100000, 999999)}`;
...
export function buildOptimizationRecord(
  ...
  return {
    regionalOptimizationId: `gri-${Date.now()}-${signals.region}-${signals.optimizationCategory}`,
    globalRiskId: `gri-risk-${Date.now()}-${signals.region}-${signals.optimizationCategory}`,
    timestamp: new Date().toISOString(),
    companyReference: signals.companyReference,
    country: signals.region,
    region: signals.region,
    riskCategory: signals.optimizationCategory,
    riskSeverity: signals.optimizationPriority,
    businessImpact: `Structural impact signal for ${signals.region}`,
    mitigationRecommendation: signals.recommendationSummary,
    ...
    riskTraceId: signals.optimizationTraceId,
