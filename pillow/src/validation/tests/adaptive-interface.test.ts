...
describe("T5-06 Adaptive Interface", () => {
  beforeEach(() => {
    resetVisualCaptureEngineForTesting();
    resetUiStateMapperForTesting();
    resetComponentRecognitionForTesting();
  ...
  test("buildAdaptiveInterfaceConfiguration loads defaults", () => {
    const config = buildAdaptiveInterfaceConfiguration(REPO_ROOT);
    assert.equal(config.enabled, true);
    assert.equal(config.recommendOnlyMode, true);
    assert.equal(config.adaptationRulesEnabled, true);
    assert.ok(ADAPTATION_CATEGORIES.includes("adaptive_layout"));
  });

  test("adaptive interface initializes with doctrine doc", async () => {
    ...
    assert.equal(state.missionId, "T5-06");
    assert.ok(ADAPTIVE_INTERFACE_SYSTEM_PATH.includes("ADAPTIVE_INTERFACE"));
    engines.visualCapture.stopCapture();
  });

  test("adapt produces machine-readable adaptive interface records", async () => {
    const engines = await buildPipeline();
    ...
    assert.ok(report.adaptationRunReportId.startsWith("aie-run-"));
    assert.ok(report.records.length > 0, `expected records, got ${report.records.length}`);
    for (const record of report.records) {
      assert.equal(record.recommendOnly, true);
      assert.ok(record.adaptiveInterfaceId.startsWith("aie-"));
      assert.ok(record.metadataVersion);
      assert.ok(record.currentWorkflowContext);
      assert.ok(record.expectedProductivityBenefit);
    }
    assert.ok(
      report.records.some((r) => r.recommendedInterfaceAdaptations.length > 0),
    );
    engines.visualCapture.stopCapture();
  });

  test("detects workflow context from T5 chain", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.adaptiveInterface.adapt();
    assert.ok(
      report.records.some((r) => r.currentWorkflowContext.length > 0),
    );
    engines.visualCapture.stopCapture();
  });

  test("generates adaptive navigation recommendations", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.adaptiveInterface.adapt();
    assert.ok(
      report.records.some(
        (r) =>
          r.adaptationCategory === "adaptive_navigation" ||
  ...
  test("generates adaptive workspace recommendations", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    const report = engines.adaptiveInterface.adapt();
    assert.ok(
      report.records.some(
        (r) =>
          r.adaptationCategory === "adaptive_workspace" ||
          r.adaptationCategory === "adaptive_panel_organization" ||
          r.recommendedWorkspaceAdaptations.length > 0,
      ),
    );
    engines.visualCapture.stopCapture();
  });

  test("never applies interface changes automatically (recommend-only)", async () => {
    const engines = await buildPipeline();
    const report = engines.adaptiveInterface.adapt();
    for (const record of report.records) {
      assert.equal(record.recommendOnly, true);
  ...
  test("start and stop continuous adaptation toggles monitoring state", async () => {
    const engines = await buildPipeline();
    const started = engines.adaptiveInterface.startContinuousAdaptation();
    assert.equal(started.health.continuousAdaptationActive, true);
    const stopped = engines.adaptiveInterface.stopContinuousAdaptation();
    assert.equal(stopped.health.continuousAdaptationActive, false);
    engines.visualCapture.stopCapture();
  });

  test("validateForSupervisorSync reports readiness after adaptation", async () => {
    const engines = await buildPipeline();
    engines.adaptiveInterface.adapt();
    const sync = engines.adaptiveInterface.validateForSupervisorSync();
    assert.ok(sync.readinessScore >= 20);
    assert.ok(["healthy", "degraded", "blocked"].includes(sync.health));
    engines.visualCapture.stopCapture();
  });

  test("cockpit snapshot exposes adaptation health", async () => {
    const engines = await buildPipeline();
    engines.adaptiveInterface.adapt();
    const cockpit = engines.adaptiveInterface.getCockpitSnapshot();
    assert.ok(cockpit.totalAdaptationCycles >= 1);
    assert.ok(cockpit.engineStatus);
    engines.visualCapture.stopCapture();
  });

  test("getLatestReport returns most recent adaptation run", async () => {
    const engines = await buildPipeline();
    const report = engines.adaptiveInterface.adapt();
    const latest = engines.adaptiveInterface.getLatestReport();
    assert.equal(latest?.adaptationRunReportId, report.adaptationRunReportId);
    engines.visualCapture.stopCapture();
  });

  test("maintains adaptive interface profile after adaptation", async () => {
    const engines = await buildPipeline();
    await primeUpstream(engines);
    engines.adaptiveInterface.adapt();
    const profile = engines.adaptiveInterface.getActiveProfile();
    assert.ok(profile);
    assert.ok(profile!.profileId.startsWith("aie-profile-"));
    engines.visualCapture.stopCapture();
  });
});
