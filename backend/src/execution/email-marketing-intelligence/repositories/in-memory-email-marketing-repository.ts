...
} from "./email-marketing-repository.js";

function recordKey(workspaceId: string, recordId: string): string {
  return `${workspaceId}:email-marketing:${recordId}`;
}
...
/** In-memory EmailMarketingRepository for Mission 084 tests and local development. */
export class InMemoryEmailMarketingRepository implements EmailMarketingRepository {
  private readonly store = new Map<string, EmailMarketingRecord>();
  ...
  async save(
    ...
    if (existingId) {
      ...
      if (existing) {
        const updated: EmailMarketingRecord = {
          ...existing,
          ...cloned,
    ...
    const record: EmailMarketingRecord = {
      recordId: randomUUID(),
      blueprintId,
      workspaceId,
      ...cloned,
  ...
  async getById(workspaceId: string, recordId: string): Promise<EmailMarketingRecord | null> {
    const record = this.store.get(recordKey(workspaceId, recordId));
    return record ? structuredClone(record) : null;
  }

  async getByStore(workspaceId: string, storeId: string): Promise<EmailMarketingRecord | null> {
    const recordId = this.storeIndex.get(`${workspaceId}:${storeId}`);
    if (!recordId) return null;
    return this.getById(workspaceId, recordId);
  }

  async list(query: EmailMarketingRepositoryQuery): Promise<EmailMarketingRecord[]> {
...
/** Factory for a fresh in-memory email marketing repository. */
export function createInMemoryEmailMarketingRepository(): InMemoryEmailMarketingRepository {
  return new InMemoryEmailMarketingRepository();
}
