/** R5-05 — YouTube Ads recovery manager. */
...
export class RecoveryManager {
  ...
  recordFailure(error: string, config: YouTubeAdsIntegrationConfiguration): boolean {
    this.consecutiveFailures += 1;
    appendYaiLog({
      event: "youtube_ads_failure",
      level: "warn",
    ...
    appendYaiLog({
      ...
      level: "info",
      details: `YouTube Ads recovery attempt ${this.recoveryAttempts}`,
    });
